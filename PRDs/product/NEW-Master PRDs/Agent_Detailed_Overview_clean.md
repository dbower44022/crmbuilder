...
*Aligned to `Agent-System-Overview.md` as updated 2026-06-20. Where this conversation used informal or draft terminology that has since been superseded, the current official term is noted.*
...
**ADO (Agent Delivery Organization)**
The little company of AI agents that builds one Planning Item at a time. Think of it like a toy factory: there's a boss agent, team-leader agents, and worker agents, all running automatically once the humans hand the work off. The ADO sits *inside* the release pipeline — the pipeline is the bigger assembly line; the ADO is the factory floor within it. Every name in the ADO ends with the word "Agent" (Project Manager Agent, PI Lead Agent, etc.) to make clear these are AI programs, not people.

---

**Agent**
A helper robot. You give it a specific job and a set of instructions, it does the job by itself with no human watching, then stops and reports back. In CRMBuilder, agents are AI programs (Claude) that write or check code. Every agent is **spawned on demand** — there are no agents sitting around running all day. The system starts one fresh for each task, the agent reads its job and memory from the database, does its work, writes back, and exits. "Standing agent" just means the registry remembers who it is; the actual agent process only runs when there's something to do.

*Naming rule: every agent's name ends in the word "Agent." If a name doesn't end in "Agent," it is not an agent — for example, the scheduler is not an agent.*

---

**Agent Profile (AGP)**
One agent's job description stored in the registry — its personnel file. It contains the agent's instructions (prompt), the hard rules it must follow, the skills it has, and which area and tier it belongs to. When an agent is spawned, the registry assembles a **contract** from this profile and hands it to the agent. Profiles are identified with codes like AGP-001 or AGP-004. Today only five profiles exist, which means most area/tier combinations have no profile at all — a major gap.

---

**Agent Profile Registry**
The system's filing cabinet of job descriptions, tools, rules, and lessons-learned for every kind of agent. It is the authoritative source of agent contracts. When a Work Task needs to be assigned, the system looks up the registry to find the right profile for that area and tier combination. Today the registry is built but lightly populated — 5 profiles, 23 tool skills, 18 rules (only 1 enforced), and just 1 learning. The "living knowledge base that gets smarter every release" is mostly capacity, not yet content.

---

**Area**
The specific kind of work a task belongs to. Every Work Task is labeled with exactly one area, and that label determines which specialist agent handles it. System areas are baked into the code and form a stack from bottom to top: storage (the database) → access (rules for reading/writing data) → api (the web service) → mcp (the AI interface) → ui (the screens people see). There are also non-code areas like methodology and infrastructure. Engagements can add their own custom areas on top of the system ones — for example, "web-ui" as distinct from the desktop "ui." Each area requires completely different knowledge and rules.

---

**Area Specialist Agent**
The agent at the bottom of the ADO hierarchy that actually does the hands-on work — writing code, editing files, making things. Each one gets its own private copy of the codebase (a worktree), does its one task, commits the result, and exits. These are the most numerous agents; REL-005 spawned 16 of them. In the target model, these evolve into **Developer Agents** and **Tester Agents** with separate, specialized roles.

---

**Architect Agent**
In the **target model** (the design direction, not yet fully built), this is the agent that decides *exactly what to build* for a given area — it produces the complete design specification before any code is written. It replaces what was called the "Phase Specialist Agent" in the older model. The Architect Agent is distinct from the Developer Agent (which builds) and the Tester Agent (which verifies independently). *In this conversation, "Decomposition Agent" (AGP-004) was used loosely for a similar planning role — but the Architect Agent in the target model is a per-area expert, not a single release-level planner.*

---

**Batch Unit**
The amount of work a single Area Specialist Agent handles in one session. Today's batch unit is too small — one agent per Work Task per Planning Item — which means the storage expert gets re-started and re-briefed separately for each tiny chunk of storage work across the release. The agreed target (option B, the original design intent) is one agent per area per phase across the whole release — so the storage expert sees all the storage work for the entire release's Develop phase at once, does it in one session, and moves on. This dramatically reduces context-reload cost and fan-out.

---

**Branch**
A private copy of the codebase that an Area Specialist Agent works in. When an agent is spawned, it gets its own branch — like a separate workbench — so it can make changes without interfering with anyone else's work. When the agent is done and its work is verified, the scheduler merges the branch back into the main codebase. Agents are prohibited from merging or pushing themselves — that authority belongs to the scheduler. If an agent is killed mid-work before committing, everything on that branch is lost.

---

**Coherence Check (Cross-Area Coherence Check)**
A review that happens after all areas finish their Design phase and before anyone starts building. Its job is to make sure the separate area designs actually fit together. For example: the api area designed a field called "phoneNumber" but the ui area designed its screen around "phone" — these don't match, and a mismatch like this would cause build failures later. The coherence check is supposed to catch disagreements before any code is written. Today the reconciliation gate that runs between Design and Develop is modest — it mainly confirms Design is finished and that no blocking finding is open. The deeper automatic cross-area coherence checking is "largely aspirational, not yet fully built" per the Overview document.

---

**Conflict**
When two requested changes want the same specific detail to be two different things at the same time. For example, one requirement says "email is required" and another says "email is optional." The reconciliation engine detects this and flags it — a human decides which way it goes. The system never quietly picks one; it always surfaces the disagreement.

---

**Contract (Agent Contract)**
The exact instruction sheet an agent gets when it starts — assembled fresh each time from the registry. It combines: the agent's job description (profile), its allowed tools and skills, its hard rules, and any lessons learned from past runs. A proper contract specifies role and scope, hard MUST/MUST NOT guidelines, a deliverable with a checkable done-condition, an operating sequence, a halt/escalate procedure, and reporting requirements. Today most contracts are loose and vague — more like a casual note than a real contract. The contract also carries a **version stamp** — a code number that says "this exact instruction sheet, this version," so you can tell if it changed between runs.

---

**Decision (DEC)**
A formal governance record in the database that captures a significant choice: what was decided, why, what alternatives were considered, and what the consequences are. Every important architectural or requirement change is anchored to a decision record so the reasoning is permanently traceable. Decisions are identified with codes like DEC-554 or DEC-556.

---

**Decomposition**
The act of breaking a Planning Item into a structured set of phases (Design → Develop → Test) and Work Tasks, with a valid ordering so no task waits on something that runs after it. A well-formed decomposition has three phases, tasks inside each phase labeled by area, and dependency arrows that only point forward. A malformed decomposition — like REL-005's, where a Design task was told to wait on a Test task — will deadlock and never complete.

---

**Delta Set**
The set of specific data changes that need to happen for a Planning Item — things like "add this field," "rename that entity," "remove this relationship." It is the output of the Reconciliation Agent: a precise, tidy list of exact changes rather than a vague wish list. The decomposition step consumes the delta set and turns it into a build plan. In REL-005, the delta set for the whole release was incorrectly handed to every Planning Item's decomposition, causing each one to plan "build everything" instead of just its own piece.

---

**Developer Agent**
In the **target model**, the agent that writes the code for one area's work — it picks up the Architect Agent's specification and implements it. This is the evolution of the current "Area Specialist Agent." The key distinction from the Architect Agent: the Developer Agent builds; it does not decide what to build.

---

**Dev Lane**
The exclusive stretch of the release pipeline (development → qa → testing → deployment) where only one release is allowed at a time. Think of it like a single-lane bridge — one vehicle crosses at a time. A release enters the dev lane by crossing the ready → development transition, which is gated: it won't open if another release is already in the lane. If a release fails QA or Test, it bounces back into development and must pass both gates again before it can ship.

---

**Engagement**
Which customer or project we're working for right now. Each engagement has its own isolated database of governance records so work for different customers never gets mixed up. Every API request carries an engagement tag (`X-Engagement`) to identify which customer's records to use. Agent profiles can be customized per engagement — so one engagement might use Angular for its web UI agent and another might use React.

---

**Fan-Out**
The number of agents spawned to do a batch of work. High fan-out means many agents running in parallel, each doing a tiny piece. Low fan-out means fewer agents, each doing a larger piece. REL-005 had excessive fan-out — 16 agents for 2 simple requirements — because the batch unit was too fine. Reducing fan-out (by batching area work across the full release) is one of the main efficiency gains from the redesign.

---

**Finding (FND)**
A sticky-note that records a problem discovered during a coherence check or review — things like "these two area designs contradict each other" or "this requirement has no design covering it." A **blocking** finding stops the build from advancing until someone resolves it. An **advisory** finding is just a heads-up; it doesn't stop anything. Findings are the currency of the reconciliation gate — the gate between Design and Develop checks that no blocking findings are open before allowing Develop to start.

---

**Fleet**
The collection of all the agent-spawned commits produced during a release's development phase. When Area Specialist Agents do their work and commit to branches, those commits accumulate as "fleet commits." In REL-005, the fleet consisted of 33 commits that had already been pushed to the shared codebase before the planning session where we discussed keeping vs. reverting them.

---

**Forensic Trace**
A detailed after-the-fact record of what went wrong during a failed or problematic run. In this session, the forensic trace documented exactly what each agent did during REL-005 — how long it ran, what it produced, where it spun, what it committed, and why things went wrong. Reading the forensic trace was the first step in understanding which code seams needed to be fixed.

---

**Freeze / Plan Freeze**
The moment a human locks the plan and hands it to the agents. It is a *step you take*, not a switch on an object — like ringing a bell to start a race. Before the freeze, planning is warm and loose (people can work on anything in parallel). After the freeze, the rules get strict: work happens in a careful, enforced order so agents never collide. The scheduler wakes up and starts driving the pipeline only after the freeze. Nothing automatic crosses the freeze line — it is always a deliberate human decision. See also: **Two Temperatures**.
...
**Governance Rule (GVR)**
A rule an agent must follow, stored in the registry. Rules come in two strengths: **advisory** (a suggestion — "you should…") and **enforced** (a hard rule that blocks the work if broken — "you must…"). Today only 1 of 18 rules is enforced. A soft suggestion is not a guardrail; the hardening work is largely about converting advisory rules to enforced ones and adding new enforced rules where none exist.

---

**Guardrail**
Any rule, check, or structural constraint that prevents an agent from going off the rails — whether it lives in the agent's contract, in the scheduler code, or in the database schema. A guardrail is only as good as its enforcement. The entire hardening project is about closing the gap between guardrails that exist on paper and guardrails that are actually enforced in code.

---

**Hardening**
The project of making the agent pipeline more reliable and predictable by fixing the gaps that caused failures like REL-005. "Hardening" means adding strict contracts, converting advisory rules to enforced ones, closing profile gaps, wiring checks that exist but aren't connected, and generally moving the system from "loosely defined and hoping for the best" to "precisely specified and mechanically enforced."

---

**Implementation Gap**
A situation where a requirement is confirmed (agreed upon and approved) but the code doesn't actually implement it yet. The coherence gate is the clearest example: REQ-027 says "Develop does not start until the cross-area coherence check reports no unresolved conflicts" — that requirement is confirmed — but the PI Lead Agent's code never runs the check. The requirement exists; the build does not. An implementation gap is different from a missing requirement.

---

**Job Card**
The informal name for an agent's stored prompt — the instructions it receives when it starts. A proper job card tells the agent its role, its hard rules, exactly what it must produce, and how it must report back. Today most job cards are loose and vague. AGP-003's job card, for example, describes steps the agent doesn't actually perform (the scheduler handles them), creating a false picture of the agent's authority.

---

**Layer A / B / C / D**
An informal four-layer framing used in this conversation to describe the architecture. It has since been superseded by cleaner terminology in the Overview document, but the layers map as follows: **Layer A** = the scheduler / release pipeline (`release_runtime.py`). **Layer B** = the two LLM planning agents (Reconciliation and Decomposition). **Layer C** = the ADO substrate (Project Manager Agent, PI Lead Agent, Phase Specialist Agent tiers — mostly deterministic code). **Layer D** = the Area Specialist Agents that do the actual work. The current preferred framing is: release pipeline → ADO → scheduler, with the registry sitting to the side supplying contracts.

---

**Learning (LRN)**
A lesson the agents picked up from doing real work and had recorded in the registry — like "last time we did a migration, watch out for X." The more times a learning proves true across runs, the higher its confidence score grows and the more weight the system gives it. Learnings are how the agent organization is supposed to get smarter over time. Today only 1 learning exists in the registry; the mechanism is built but largely empty.

---

**Live Records**
The actual rows in the governance database as they exist right now, not a cached snapshot. Every time the scheduler decides what to do next, it queries the database fresh. If an agent just updated a Work Task to Complete two seconds ago, the scheduler sees that on its very next read. This is also why two sessions touching the same records at the same time is hazardous — they are writing to the same live rows.

---

**Merge**
The act of taking the work an Area Specialist Agent did on its private branch and combining it back into the main shared codebase. The scheduler (not the agent) performs the merge after verifying the agent's work passed the tests. Agents are explicitly prohibited from merging or pushing — that authority belongs to the scheduler.

---

**Migration Lock**
A special pause-everyone lock used only when the database's shape (its schema) needs to change. While the migration lock is held, every other agent is paused — that risky operation runs completely alone so nobody is reading a half-changed database structure. Regular file locks (used for normal coding work) are separate and don't pause everyone.

---

**Needs Attention (needs_attention)**
A flag an agent raises to signal "I hit a problem I can't resolve — a human needs to look at this." It is the most important help signal in the whole system. Today agents have no direct path to set this flag themselves — only the scheduler sets it after the fact when it detects something went wrong. A properly designed agent would set it proactively when it discovers a mis-scoped task, a duplicate, a missing dependency, or an absent done-condition, and then stop rather than produce filler work.

---

**Operating Protocol**
The step-by-step procedure an agent follows during its run: (1) claim the task, (2) do the work, (3) commit, (4) verify, (5) mark Complete, (6) exit. The current protocol is missing a "Step 0: check if this work is already done" and has no time budget on verification — which is exactly why REL-005's agent could spin for 30 minutes on a test suite before being killed. A corrected protocol adds Step 0 validation, moves commit to before verify so a kill can never discard work, and adds a time budget with an "on overrun → commit and report what's done" instruction.

---

**Phase**
One of the three sequential stages that every Planning Item's work passes through: Design (figure out exactly what to build), Develop (write the code), and Test (prove it works). All areas must complete their Design tasks before any area starts Develop, and all areas must complete Develop before any area starts Test. The phase boundaries are what make the cross-area coherence check possible — all areas finish designing before anyone starts building. In the database, each phase is stored as a **Workstream** record.

---

**Phase Gate**
The check that must pass before the pipeline advances from one phase to the next. Today the gate between Design and Develop only checks "are all Design tasks marked Complete?" — a weak criterion. A properly hardened gate would also run the cross-area coherence check (confirming all area designs actually agree) and verify each task met its acceptance criterion, not just that someone marked it done.

---

**Phase Specialist Agent**
The tier-3 agent in the current ADO structure. It writes the to-do list for one phase — breaking a big phase into small, single-topic Work Tasks. In the **target model**, this role evolves into the **Architect Agent**, which is a per-area expert rather than a single generic planner.

---

**PI Lead Agent**
The tier-2 agent in the ADO. It takes one Planning Item and walks it through its phases in order — opening each phase when the prior one completes, declaring phases complete when all their tasks are done, and raising `needs_attention` flags when something goes wrong. It is the component that should run the coherence check between Design and Develop, but currently doesn't. Despite being called an "agent," the PI Lead today is largely deterministic substrate code rather than an LLM.

---

**Pipeline**
The fixed sequence of stages a release moves through from initial planning all the way to shipped: preliminary_planning → development_planning → reconciliation → architecture_planning → ready → development → qa → testing → deployment → shipped. The pipeline position is stored in a single `release_status` field on the release record. The scheduler drives the release through this pipeline one step at a time, and a release can only move to states on the approved-transitions list — it can't skip stages.

---

**Planning Item (PI)**
One unit of work inside a project — roughly one feature or one requirement's worth of work to build. Think of it as an item on a to-do list. A release is made up of one or more Planning Items. Each Planning Item goes through its own Design → Develop → Test cycle inside the ADO. Planning Items are identified with codes like PI-230 or PI-231.

---

**Profile Gap**
The situation where a Work Task arrives labeled with an area (like "ui") but no agent profile exists for that area and tier combination. Today only five profiles exist (AGP-001 through AGP-005), covering only storage and a few planning-level roles. There are zero profiles for ui, access, api, mcp, or any tester role. When a ui task arrives with no ui profile, the system falls back to the storage developer's card — and the UI agent boots with database-flavored instructions and no UI-specific rules at all. The fix is to refuse the task rather than misroute it.

---

**Project (PRJ)**
A big, long-running theme of work — like "the kitchen remodel." It holds lots of smaller Planning Items. When a release is assembled, whole Projects are added to it, and each project carries its Planning Items along. *Note: the word "Workstream" was previously used to mean what is now called a Project — it was recycled to mean something different (a phase container). If you see "Workstream" in older documents, check which meaning is intended.*

---

**QA Gate**
The release-level check that runs after all Planning Items are built, before the release enters testing. It asks: does the design actually cover every confirmed requirement, with no contradictions? If yes, the release advances to testing. If no, it bounces back to development. Nothing ships without passing this gate.

---

**Reconciliation Agent (AGP-003)**
The first LLM agent in the pipeline. The scheduler hands it the full list of confirmed requirements for the release, and it produces a reconciled demand-set: a precise, tidy list of exact data changes needed. If two requirements contradict each other, it is supposed to flag the conflict rather than quietly pick one. The scheduler then does the actual persisting and conflict-resolution — the agent only produces the change-list. Today its job card describes several steps it doesn't actually perform, creating a misleading picture of its authority.

---

**Reconciliation Engine**
The mathematical component that merges everyone's requested changes into one clean change-list and flags any spot where two changes want the same thing to be different values (a conflict). It always gives the same answer for the same input — it's deterministic, not an AI. The reconciliation engine runs during the Architecture Planning stage of the release pipeline, before any building starts.

---

**Reconciliation Gate**
The checkpoint between Design and Develop inside the ADO. You only start building once the design is settled and there are no blocking findings. Today the gate is modest — it mainly confirms Design is done and no blocking finding is open. The deeper automatic cross-area coherence checking is largely aspirational and not yet fully built.

---

**Registry**
See **Agent Profile Registry**.

---

**Release**
A bundle of finished Planning Items that ship together — like packing several wrapped presents into one box to deliver at once. A release has a scope (which Projects and Planning Items it includes), a pipeline status (where it is in the development process), and a governance record in the database. Only one release can be in the dev lane at a time. Releases are identified with codes like REL-005.

---

**Requirement (REQ)**
A formal, database-stored statement of something the system must do, with an acceptance criterion that makes it testable. Requirements have lifecycle states (candidate → confirmed → deferred / rejected) and can be linked to the decisions that created or changed them. In this session, several new requirements were captured (REQ-277 through REQ-283) and several existing confirmed requirements were amended when their scope became stale. Human approval is required before any requirement is confirmed — nothing gets built from an unconfirmed requirement.

---

**Scheduler**
The program that drives the whole release pipeline — starting agents at the right moment, checking their results, running the gates, and stopping to ask a human whenever something needs attention. Think of it like an orchestra conductor waving the baton (but the Overview document is careful to note that "conductor" is just an analogy, not a second name). The scheduler is *not* itself an agent — it carries no "Agent" suffix — because it is the program that *spawns* agents, not one of them. There are several scheduler programs working at different levels: `release_runtime.py` for the overall pipeline, `ado_runtime.py` for individual Planning Items, and `parallel_runtime.py` / `coordinating_runtime.py` for the pool of Area Specialist Agents.

---

**Seam**
A point in the code where deterministic substrate hands off to an LLM agent — where a program stops doing math and starts asking an AI to make a judgment. There are four seams in the current pipeline: the demands/reconciliation call, the decomposition call, the area specialist spawns, and the QA/test gates. "The code seam" is the precise location in the source where a gap or fix lands.

---

**Skill (SKL)**
Something an agent knows how to do, or a tool it's allowed to use — stored in the registry as part of the agent's profile. Skills are what specialize one agent type from another. A storage agent's skills include reading schema files and writing Alembic migrations; a UI agent's skills would include Qt widget conventions and palette rules. Today the registry has 23 tool skills, but they are not yet finely differentiated by area.

---

**Spawn / Spawn-on-Demand**
The act of starting a fresh agent instance for a specific task. The scheduler spawns an agent by creating a worktree, looking up the right profile from the registry, assembling the contract, and launching the agent process. Each spawned agent is independent and isolated. Crucially, agents are not left running between tasks — every agent is started fresh, does its work, and exits. "Standing" in the registry just means the database remembers who the agent is; the actual running process only exists while there's work to do.

---

**State Machine**
A system that can only be in one state at a time and can only move between states in pre-approved ways. The release pipeline is a state machine: a release is always in exactly one of its 12 pipeline states, and it can only move to states on the allowed-transitions list. You can't jump from "development" directly to "shipped" — you must go through qa, testing, and deployment first. This rigidity is intentional; it makes the pipeline predictable and prevents states from being skipped or faked.

---

**Tester Agent**
In the **target model**, the agent that independently verifies the Developer Agent's work without having been involved in writing it. The key word is *independently* — the Tester Agent checks the code honestly, without the bias of having written it. This is a new role that doesn't exist in the current built system, where verification is handled by the scheduler's post-agent test run.

---

**Tier**
The level of expertise an agent has within its area. The three tiers are Architect (designs the solution), Developer (builds it), and Tester (verifies it works independently). For build areas (storage, access, api, mcp, ui), all three tiers are needed. For design-only areas (methodology), only the Architect tier applies. A Work Task's area label and tier together determine which exact agent profile should handle it — today most tier/area combinations have no profile at all.

---

**Topic (TOP)**
A hierarchical grouping of requirements in the V2 database used for review and organization. Topics are lenses — they group related requirements together, but they themselves are never built. The agent guardrails work lives under TOP-099; observability under TOP-100; the broader Agent System under TOP-005. Requirements can belong to one topic and also refine requirements in other topics.

---

**Two Temperatures**
The name for the planning model that splits work into two modes: **warm** (before the freeze — loose, parallel, people can work on anything) and **cold** (after the freeze — strict, orderly, one thing at a time so agents never collide). The freeze is the dividing line. Before it, humans plan freely. After it, the scheduler enforces the order. The two-temperature model is how the system prevents agents from interfering with each other's work.

---

**Verification / Verify**
The act of checking that an agent's work is correct before accepting it. There are two kinds discussed in this session: (1) the scheduler's post-agent check (did the task get marked Complete? did the branch get commits? do the affected tests pass?), and (2) the agent's own self-check before marking itself done (today: run the linter and tests). The core architectural question was whether agents should self-verify or whether the scheduler should own the test gate. The answer leaned toward scheduler-owns, because agent-side self-verification is exactly where the 30-minute spin disaster in REL-005 occurred.

---

**Worktree**
A separate, isolated copy of the codebase created for one agent's use. When an Area Specialist Agent is spawned, it gets its own worktree — like a private sandbox — so it can make changes without touching anyone else's work. When the agent is done and its work is verified, the scheduler merges the worktree's branch into main and discards the worktree. If an agent is killed before committing, everything in its worktree is lost.

---

**Work Task (WTK)**
The smallest unit of work in the system: one specific piece of work, in one area, sized for a single agent session (or in the target batch model, one area-phase pass across the whole release). A Work Task belongs to exactly one phase (Design, Develop, or Test) of exactly one Planning Item, and is labeled with exactly one area. A properly designed Work Task also carries an acceptance criterion written by the Architect Agent at planning time — without one, the worker agent has no way to know if it succeeded. Work Tasks are identified with codes like WTK-174 or WTK-176.

---

**Workstream (WSK)**
The database record for one phase of a Planning Item — Design, Develop, or Test. When a Planning Item is decomposed into phases, three Workstream records are created. Work Tasks live inside Workstreams. The PI Lead Agent manages the Workstream lifecycle — opening and closing each phase as it progresses. Workstreams are identified with codes like WSK-150 or WSK-154.

*Naming note: "Workstream" was previously used to mean what is now called a Project (PRJ) — a long-running theme of work. The word was recycled. If you see it in older documents, context tells you which meaning applies.*
