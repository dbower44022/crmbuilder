...
- **DEC-138 — `reference_book_kind` closed enum with eleven values.** Captures the eleven-value closed enum covering DEC-117's seven artifact types plus three additional types observed in the project's operating history (`workstream_master_plan`, `apply_script`, `session_startup_document`) plus an `other` sentinel.
