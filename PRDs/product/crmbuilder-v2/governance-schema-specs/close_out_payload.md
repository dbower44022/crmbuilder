...
**Status:** Draft v1.1 — produced by schema-design conversation; v1.1 cross-spec consistency reconciliation per build-planning conversation (SES-055) §7.2 finding
...
| 1.1 | 05-22-26 17:30 | Doug Bower / Claude (build-planning conversation, SES-055) | Cross-spec consistency reconciliation per build-planning conversation §7.2 finding. Two paragraphs in §3.3.2 and §3.4.3 carried at-most-one inbound `deposit_event_applies_close_out_payload` edge language that DEC-158 (SES-054 deposit_event Decision 2 — multi-event-per-payload precedent) supersedes. Section 3.8.3 of v1.0 explicitly invited this relaxation. Substantive changes: §3.3.2 cardinality column updated from "each close_out_payload has at most one inbound edge of this kind, enforced at the access layer" to "each close_out_payload may carry zero-or-more inbound edges of this kind (one per apply attempt); the access layer does not enforce at-most-one"; §3.4.3 applied-requires-edge rule augmented with the first-success-transitions semantics — the `ready → applied` transition fires on the first inbound `deposit_event_applies_close_out_payload` edge from a deposit_event record with `outcome = 'success'`; subsequent inbound edges (failures, re-confirmations) are admitted without further close_out_payload state change. The applied-requires-edge rule itself stands: setting status to `applied` still requires at least one such inbound edge. No design or semantic substance affected beyond the cardinality relaxation; the DEC-149 applied-requires-edge precedent is realized as intended by the deposit_event-side authorship of the inbound edge. Identified and settled by SES-054 Decision 2 (DEC-158); executed in this conversation per the no-inline-amendment discipline from the schema-design phase. |

---

## Change Log

**Version 1.1 (05-22-26 17:30):** Cross-spec consistency reconciliation per the build-planning conversation's §7.2 finding (SES-055). DEC-158 (SES-054 deposit_event Decision 2 — multi-event-per-payload precedent) settled in deposit_event.md that close_out_payload records admit zero-or-more inbound `deposit_event_applies_close_out_payload` edges (one per apply attempt) rather than the safe-default at-most-one this spec's v1.0 carried. Section 3.8.3 of v1.0 explicitly invited the relaxation. Two paragraphs are revised in place: §3.3.2 cardinality column language and §3.4.3 applied-requires-edge rule are updated to reflect zero-or-more semantics with first-success-transitions clarification. The applied-requires-edge rule itself stands — setting status to `applied` still requires at least one inbound `deposit_event_applies_close_out_payload` edge; the change is the cardinality ceiling, not the floor. Subsequent inbound edges (failures, re-confirmations from re-applies) are admitted without further close_out_payload state change. Identified and settled by SES-054 Decision 2 (DEC-158); executed in this conversation per the no-inline-amendment discipline from the schema-design phase. The §3.8.3 entry that invited the relaxation is retained verbatim as historical record of the deferral.
...
|-------------------|--------|--------|-----------|-------------|-----------|
| `deposit_event_applies_close_out_payload` | `deposit_event` | `close_out_payload` | references-table edge | many-to-one (each successful or unsuccessful apply produces one deposit_event linking to one close_out_payload; each close_out_payload may carry zero-or-more inbound edges of this kind, one per apply attempt) | The deposit_event records the application of this close_out_payload to the database. **Required-when** rule from close_out_payload's side: when close_out_payload.status = `applied`, at least one inbound edge of this kind from a deposit_event record with `outcome = 'success'` must be present (see section 3.4.3 for the applied-requires-edge rule). **Zero-or-more** cardinality (revised v1.1 per SES-054 DEC-158): multiple inbound edges are admitted to capture re-apply attempts (initial apply plus subsequent verifications, re-applies after failure, or repeat success confirmations). The first inbound edge with `outcome = 'success'` drives the `ready → applied` transition; subsequent inbound edges (failures, re-confirmations) are admitted without further close_out_payload state change. The deposit_event entity is born-terminal append-only (DEC-156); apply attempts are facts and all are recorded. |

This table is informational from `close_out_payload.md`'s perspective. The `vocab.py` registration of `deposit_event_applies_close_out_payload`, the `_kinds_for_pair` clause binding it to the `(deposit_event, close_out_payload)` pair, and the zero-or-more-inbound-edges-per-target access-layer posture belong to the `deposit_event.md` schema-design conversation (DEC-158 settles the cardinality; this spec's v1.1 reconciliation reflects that settlement). This spec lists the kind here because the applied-requires-edge rule in section 3.4.3 names it, and because the cross-spec consistency check before build-planning relies on both specs naming the relationship.
...
Setting `close_out_payload_status` to `applied` requires the record to have at least one inbound reference edge of kind `deposit_event_applies_close_out_payload` from a deposit_event record with `outcome = 'success'`. The access layer enforces this as a single combined validation, mirroring the consumed-requires-edge pattern locked by DEC-143:

- POST creating a record with `status = 'applied'` and no inbound `deposit_event_applies_close_out_payload` edge from a `success` deposit_event: HTTP 422 `{"error": "applied_payload_requires_successful_deposit_event_edge"}`.
- PUT or PATCH transitioning an existing record to `status = 'applied'` without such an inbound edge present: same 422.
...
- The canonical authoring path is the apply script's POST to `/deposit-events` with `outcome = 'success'`; the access layer atomically creates the deposit_event record, creates the inbound `deposit_event_applies_close_out_payload` edge, and transitions the target close_out_payload from `ready` to `applied` in one transaction. The deposit_event spec section 3.4.3 documents the apply-script-side authoring path that drives this transition.
- DELETE on any inbound edge while the target record still has `status = 'applied'`: HTTP 422 `{"error": "applied_payload_requires_successful_deposit_event_edge"}` — the rule applies at the existential level (at least one inbound edge from a success deposit_event must remain present). The status must be changed first (e.g., via supersession with a new payload addressing the same apply target) or the target close_out_payload must be soft-deleted.

**First-success-transitions semantics** (v1.1 reconciliation per DEC-158). Multiple inbound edges are admitted under DEC-158's multi-event-per-payload precedent. The behavior:

- **First inbound edge with `outcome = 'success'` against a `ready` payload**: drives the `ready → applied` transition. The transition timestamp is set to the deposit_event's `_created_at`; the close_out_payload's `_applied_at` lifecycle timestamp is populated.
- **Subsequent inbound edges, regardless of outcome** (against an already-`applied` payload): admitted without close_out_payload state change. The payload stays at `applied`; the lifecycle timestamps are not re-modified. The new deposit_event records co-exist as additional inbound edges; the historical sequence is queryable via the deposit_event records' `_created_at` ordering.
- **Inbound edge with `outcome = 'failure'` against a `ready` payload**: admitted as a `failure`-recording deposit_event; the close_out_payload stays at `ready`. The `ready → applied` transition only fires on `success` outcome edges, never on `failure` ones. A subsequent `success` apply against the same payload drives the transition.
- **Inbound edge with `outcome = 'failure'` against an `applied` payload**: admissible under DEC-158 multi-event semantics (a verification re-apply might fail even after the initial success); admitted as a `failure`-recording deposit_event with the payload remaining at `applied`. The close_out_payload's status is not retracted.

**Cross-spec precedent realized.** This rule is the first downstream application of the terminal-state-consumption-requires-edge precedent locked by DEC-143 (work_ticket's consumed-requires-edge). The structural parallel is exact: work_ticket transitions to `consumed` require the inbound `conversation_opens_against_work_ticket` edge from the consuming conversation; close_out_payload transitions to `applied` require at least one inbound `deposit_event_applies_close_out_payload` edge with `outcome = 'success'` from the applying deposit_event. Both terminal states are defined by an external act (consumption / application); the edge naming the actor must be present before the status transition is admitted. The naming differs ("consumed" vs "applied") because the verb-orientation of the acting relationship differs, but the rule pattern is identical. Where work_ticket's pattern remains at-most-one inbound consumption edge (a work_ticket is consumed exactly once, by the family-defining single-use property), close_out_payload's pattern relaxes to zero-or-more inbound edges (a payload may be re-applied, with each apply recorded as a separate deposit_event).
