...
You are building **PI-418** ("Separate writable layouts from portal- and role-bound
variants") in PRJ-120 (Publish emitter completion). Read the PI, REQ-520 and REQ-519
from the store before writing any code — the store record is the authority, this file
is orientation.

## Governance position (verified 2026-09-03)

- **Requirement-first is already satisfied.** REQ-520 ("A construct the platform cannot
  write stays non-actionable and says why") is `confirmed` (approved by DEC-943, refines
  REQ-519). PI-418 implements it. REQ-519 ("An operator can act on role, team, layout
  and filtered tab differences") is `confirmed`; its layout half is the only part still
  open now that PI-417 (roles, teams, filtered tabs) resolved on merge `ca1b3b6b`.
- **PI-418 is `Draft`** with no claimant, no branch, no worktree. Set it In Progress
  when you start (claim it), not before.
- **PI-427** ("Emit and publish layout definitions", `Draft`, PRJ-120, implements
  REQ-519, recorded by DEC-951 and rehomed by DEC-985) is the sibling this item leans
  on: PI-418 offers *publish* on the writable layout subset, and nothing can be
  published until the emitter renders a `layouts:` block, which is PI-427's whole
  scope. The first design decision below is whether the two build together.
...
- Every code commit: `Governed-By: PI-418` trailer (or `PI-427` for a commit that is
  purely the emitter — one PI per commit, never both). The PreToolUse hook checks the
  *command text* for a line starting `Governed-By:` — use
  `git commit -F - -- <paths> <<'EOF' … EOF` with the trailer at column 0, and
...
- Model A: build on branch `pi-418` in a worktree
  (`git worktree add ../crmbuilder-wt-pi418 -b pi-418 main`), merge to `main` locally
  after the demonstration, **Doug pushes**, and only after his push do you resolve the
  PI in the store (`resolution_reference: "merge <sha> (pushed)"`, verified with
  `git merge-base --is-ancestor <sha> origin/main`). Copy `uv.lock` from the main clone
  into the worktree before `uv sync` (gitignored; a fresh resolve pulls mcp 2.x, which
  breaks `mcp.server.fastmcp`). Remove the worktree at close-out; keep the branch.
...
- **GVR-239 review gate:** Doug reviews each increment before merge — his preference is
  a live demonstration: short scripts under `/tmp/pi418/stepN.sh`, one command per
  step, against a scratch SQLite store (`CRMBUILDER_V2_DB_PATH=/tmp/pi418/v2.db`,
  `CRMBUILDER_V2_DATABASE_URL=""`), never the cloud store and never a live CRM, plus a
  standing HTML runbook artifact. `/tmp/pi417/generate.py` and `step1..7.sh` are the
  worked pattern from PI-417 — copy the seeding and the in-process `TestClient` API
  calls. The `instruction-discipline` skill at `~/.claude/skills/instruction-discipline`
  governs every step-by-step reply: numbered steps, where/what/expected/if-not, no bare
  code blocks, no multi-line paste.
...
- **LSN-071:** a new emitted block needs at least one test that starts at
  ...
  the validation gate both passed while the publish rendered nothing.
...
- **Layouts are captured, compared, and held entirely view-only.**
  ...
  (its docstring cites REQ-520 as the reason — the blanket default this PI replaces).
...
- **The audit reads the 18 ordinary types only.** `introspect/reconcile.py::
  ...
  five portal types (`espo_impl/core/layout_types.py::PORTAL_LAYOUTS`) and never fetched
  them either; role binding in EspoCRM 9.x is Layout Sets bound to Teams (schema
  §12.5.2, DEC-6), which nothing in V2 reads.
...
- **No `layouts:` block is emitted.** `adapters/espocrm/model.py` has no layout
  ...
  layouts — this is PI-427. The translator you need already exists in reverse: the V1
  audit's `espo_impl/core/reconcile/capture.py::_reverse_detail_layout /
...
- **Publish scope:** `reconcile_apply.py::entity_for_member` already routes a layout to
  its entity, so `publish_scope_for_member` works for layouts the moment a block exists.
  A layout's *presence* row should follow the field/filtered-tab precedent (view-only —
  its entity's promote carries it), not the role/team one.

## Design decisions to put to Doug (one per message, options + recommendation)

1. **Does PI-427 build in this session?** Without a `layouts:` block, PI-418 can open
   *capture* on ordinary layouts but not *publish*, and REQ-519's layout half stays
   unsatisfied. Options: (A) build PI-427's emitter first in this session, then PI-418
   on top, two commits with their own `Governed-By`; (B) PI-418 capture-only now,
   publish waits for a separate PI-427 session; (C) PI-427 alone first, PI-418 after.
   Recommend A: the two are one increment to a user ("layouts publish") and the
   demonstration needs both to show a layout going out.
2. **How the unwritable subset becomes visible.** REQ-520 says a portal- or role-bound
...
3. **Where the reason lives.** The non-actionable row must "name why". Options: (A) a
   per-type reason table in `reconcile_compare` surfaced on the row (`capability_reason`)
   and in the UI's refusal noun (`_MEMBER_TYPE_NOUNS` pattern in `reconcile_grid.py`);
   (B) a note on the construct-set declaration only. Recommend A: the operator reads the
   row, not the declaration.
4. **Confirmed-only emission for layouts.** 257 CBM layouts are candidates. Emit only
   `confirmed` (every other construct's rule) or treat audit-captured layouts as
   emit-worthy at candidate? Recommend confirmed-only and a note owed to Doug that CBM's
   layouts need a confirmation pass before a layout publish there means anything.

## Suggested build order

1. Read PI-418, PI-427, REQ-519/520, DEC-943/951/985/1001, LSN-069/070/071 from the
   store; verify the governance position above still holds; claim PI-418.
2. Worktree + branch `pi-418`; copy `uv.lock`; `uv sync`.
3. Decision 1 with Doug — it decides whether steps 4–5 happen here.
4. (PI-427) `list_layouts` on the DesignClient protocol and all three clients (parity
   test `METHODS` count goes to 17 and its seed grows a layout); `_apply_layouts` in
   `model.py` rendering `layout:` per entity from `layout_content` via the reverse
   translators, strict field resolution, `autoPlaceName` respected; `generate_design_yaml`
   passes it; `validate_yaml_text` green on the emitted program; the LSN-071 test.
5. (PI-418) split the type: writable subset gets `(True, True)` attribute capability and
   a `layout` entry in `_GLOBAL_MEMBER_REPOS` (get/patch `layout_content`); unwritable
   subset stays `(False, False)` with a reason; presence rows view-only; decision 2's
   capture extension; the compared-set/conformance declarations made consistent per
   subset (an unwritable variant's drift is `named_but_unwritable`, an ordinary
   layout's is `drifted`). Update the pins: `test_layout_attributes_stay_view_only`,
   `test_compared_set.py`, `test_conformance.py`, and the API/UI tests that list
   member capabilities.
6. Demonstration scripts under `/tmp/pi418` (must include: an emitted `layout:` block
   on a generated program that passes the validator; a writable layout row showing
   capture and publish; an unwritable variant row showing no action and the reason;
   the publish-scope routing to the entity's program; the conformance verdict for each
   subset), the standing HTML runbook, and Doug's walkthrough.
7. Commit (pathspec + trailer), merge to main after the demonstration, re-run the
   slices on merged main, Doug pushes, resolve PI-418 (and PI-427 if built) in the
   store, close the session, remove the worktree.
...
- Layout Sets (team-bound layout variants) are unrepresented in V2 end to end; if
  decision 2 lands on option A, record the Layout Set gap as a requirement candidate.
