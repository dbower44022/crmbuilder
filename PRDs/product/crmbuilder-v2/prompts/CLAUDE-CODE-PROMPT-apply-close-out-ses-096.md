# CLAUDE-CODE-PROMPT — apply close-out SES-096 (governance-recording-rules.md v0.1 + CLAUDE.md wiring + DEC-299 conceptual alignment)
...
**Slice:** Apply the SES-096 close-out payload to the V2 governance DB
...
Apply `PRDs/product/crmbuilder-v2/close-out-payloads/ses_096.json` to the V2 governance DB via the standard apply script. Creates:

- **SES-096** — session record for governance-recording-rules.md v0.1 authoring
- **CONV-066** — conversation record (status=`complete`) with two embedded reference edges (`conversation_belongs_to_workstream` → WS-011; `conversation_records_session` → SES-096)
- **DEC-315** — Rules document lives at `specifications/governance-recording-rules.md`, not under `PRDs/process/conduct/`
- **DEC-316** — Workstreams covered as first-class section; all record creation via API or MCP, never via desktop UI
- **DEC-317** — Adopts DEC-299 conceptual model for Session/Conversation; codifies Mandatory Logging as a core principle
- **DEC-318** — Multi-conversation Claude.ai chats handled via PI-073 schema evolution (Option B), not via one-V2-session-per-conversation (Option A)
- **PI-090** — Apply-pipeline validator script for governance-recording-rules conformance (status=Open)
- **8 reference rows:**
  - `decided_in` DEC-315 → SES-096
  - `decided_in` DEC-316 → SES-096
  - `decided_in` DEC-317 → SES-096
  - `decided_in` DEC-318 → SES-096
  - `is_about` DEC-317 → DEC-299 (DEC-317 adopts DEC-299's conceptual model)
  - `is_about` DEC-318 → PI-073 (DEC-318 picks Option B which PI-073 enables)
  - `addresses` PI-090 → DEC-310 (validator delivers the enforcement layer DEC-310's mandate implies)
  - `decided_in` PI-090 → SES-096 (PI created in this session)
...
3. If a resolving PI is identified, edit `PRDs/product/crmbuilder-v2/close-out-payloads/ses_096.json` and add the identifier to `resolves_planning_items[]` (and remove from any wrong slot) **before** running apply. Equivalently, add to `addresses_planning_items[]` if the work advances but does not fully resolve.
...
5. Payload exists:
   ```bash
   ls -la PRDs/product/crmbuilder-v2/close-out-payloads/ses_096.json
   ```
...
7. **Pre-apply identifier-head capture per DEC-300** (re-verify against live API; sandbox heads were captured at session start hours earlier and may have drifted):
   ...
   ```
   Expected heads (sandbox-captured at session start): SES-094, CONV-064, DEC-313, PI-089, WT-055. If any head is now ≥ the identifier this payload reserves (SES-096, CONV-066, DEC-315..317, PI-090), the payload must be **renumbered** before apply — see Renumbering below.
...
2. Edit `ses_096.json` and update **every** internal reference: session.identifier, conversation.conversation_identifier, decisions[].identifier, planning_items[].identifier, references[].source_id and target_id, the conversation's embedded references, and the apply prompt filename (this file).
...
uv run python scripts/apply_close_out.py ../PRDs/product/crmbuilder-v2/close-out-payloads/ses_096.json
```

Expected OK counts on success:
- 1 session created (SES-096)
- 1 conversation created (CONV-066)
- 4 decisions created (DEC-315..317)
...
1. Identifier-head advancement:
   ...
   ```
   Expect SES head = SES-096, CONV head = CONV-066, DEC head = DEC-318, PI head = PI-090 (or higher if heads advanced before apply and the payload was renumbered).

2. Reference count delta — expect +10 reference rows total (8 top-level + 2 embedded):
   ```bash
   curl -sf "http://127.0.0.1:8765/references?source_id=SES-096" | python3 -c "import sys,json; print('refs targeting SES-096:', len(json.load(sys.stdin)['data']))"
   curl -sf "http://127.0.0.1:8765/references?target_id=SES-096" | python3 -c "import sys,json; print('refs targeting SES-096 (target):', len(json.load(sys.stdin)['data']))"
   ```

3. Spot-check SES-096 and one decision:
   ```bash
   curl -sf http://127.0.0.1:8765/sessions/SES-096 | python3 -m json.tool | head -30
   curl -sf http://127.0.0.1:8765/decisions/DEC-317 | python3 -m json.tool | head -30
   ```
   Expect SES-096.title and DEC-317.title to match the payload.

4. Spot-check a `decided_in` reference resolution:
   ```bash
   curl -sf "http://127.0.0.1:8765/references?source_id=DEC-317&relationship=decided_in" | python3 -m json.tool
   ```
   Expect one row, target_type=session, target_id=SES-096.
...
git add PRDs/product/crmbuilder-v2/close-out-payloads/ses_096.json
git add PRDs/product/crmbuilder-v2/prompts/CLAUDE-CODE-PROMPT-apply-close-out-ses-096.md
git add PRDs/product/crmbuilder-v2/deposit-event-logs/ 2>/dev/null || true
git status --short
git commit -m "v2: SES-096 close-out applied — governance-recording-rules.md v0.1 + CLAUDE.md wiring + DEC-299 conceptual alignment

Applied close-out payload for SES-096:

- 1 session (SES-096): governance-recording-rules.md v0.1 authoring,
  CLAUDE.md wiring, DEC-299 conceptual alignment, stop-and-log discipline.
- 1 conversation (CONV-066).
- 4 decisions (DEC-315 location/filename; DEC-316 workstreams as
  first-class plus API/MCP-only creation; DEC-317 DEC-299 conceptual
  alignment plus Mandatory Logging; DEC-318 Option B for
...
- The next-conversation kickoff path (TBD — no follow-on kickoff was authored in SES-096; PI-090's validator-build session and any rules-document trim-pass session are candidates Doug to schedule)

---

## Known caveats and follow-on items surfaced by SES-096
...
3. **Workstream linkage assignment.** Conversation CONV-066 belongs to WS-011 (V2 storage API refinements) per pragmatic choice — closest existing workstream match. A dedicated "Governance recording enforcement" workstream (provisionally WS-013) is referenced in PI-090's description as future scope. Doug may want to retro-assign CONV-066 to a new workstream once created.
