...
3. **Trace to the already-approved requirements — do NOT author new ones.** The requirements for this scope **already exist and are confirmed**: **`REQ-128`** (audit + deploy role-aware visibility = §12.5) and **`REQ-129`** (audit + deploy field-level permissions = §12.7), both under **PRJ-024** (Audit-to-Deploy Round-Trip Completeness), `requirement_status=confirmed` / `requirement_review_state=current`, human-defined and already signed off in the Review panel. Per DEC-547 these RBAC-deploy gaps are delivered via **this** PI-051 design pass rather than separate PRJ-024 PIs. So the deliverable is: confirm REQ-128/129 capture the chosen scope (refine their text via the API only if the design narrows them — a meaning change re-opens review), and add `planning_item_implements_requirement` edges from PI-051 (and from any implementing PIs the decomposition spawns) to REQ-128 and REQ-129. **The requirements gate is already satisfied** — do not POST new requirements for this scope; that would duplicate REQ-128/129. (Aside: PI-051's stale topic anchor `TOP-065` does not resolve — attach a valid topic while you're in there.)
...
Once the design lands (the requirements REQ-128/129 are **already** approved, so the only remaining gate is the design decision + the implements edges), PI-051 becomes ADO-ready via the normal path: **decompose → scope into work_tasks (area `espo`/`automation`) → Ready → dispatch-approve → ADO builds.** Until then PI-051 stays `Draft` in PRJ-017 (`in_flight`) and is correctly *not* dispatchable.
