...
**Domain overviews (09-03-26):** a domain's overview is authored as the `domain` record in the V2 store (purpose, description, notes and its persona/entity links) and rendered to `specifications/{domain-name-slug}/domain-overview.md` as a dated file with a frontmatter table and change log (DEC-1021). The first is Governance Recording, DOM-012, the methodology's ninth and cross-cutting domain (DEC-1003): `specifications/governance-recording/domain-overview.md`. Persona records are first-class and cross-domain, so their renders live in `specifications/personas/` rather than in a domain folder; the first is `specifications/personas/governance-recording-personas.md` (PER-013..015, PI-086). The store is the source; when the file and the store disagree, re-render.

**Deployment automation (09-24-26):** SES-433 recorded nine decisions, DEC-1152..DEC-1160, that reshape deployment. The model becomes **client → application → deployment** (DEC-1155, superseding DEC-922; glossary TERM-070..075, TERM-005 and TERM-067 amended): a client is any organisation using CRMBuilder, an application is one client's complete definition (private or public), a deployment is one installation for one client on its own hosting provider, holding its credentials, setting values, secrets, readiness list, release and update policy. The custom applications' settings list is defined in the store as system settings valued per deployment (DEC-1152); every CRM is published from the design, with screen code and the two add-on products as versioned components (DEC-1153); CRMBuilder deploys the chapter custom applications to App Platform through the API (DEC-1154); each client chooses its update policy and may decline or schedule a release (DEC-1156); secrets live encrypted in the store with a handover file to the client's vault (DEC-1157); person-only steps form a readiness list checked before the run (DEC-1158); client people sign in on a web surface and see only their own client (DEC-1159); Boston finishes by hand as a test system (DEC-1160). Nothing is built until the requirements session in `PRDs/product/crmbuilder-v2/kickoff-deployment-automation-requirements.md` confirms requirements and planning items.
...
- **MCP connector** `https://mcp.crmbuilder.ai` (claude.ai and Claude Code): the HTTP MCP server runs on Doug's machine as the user service `crmbuilder-v2-mcp` behind a Cloudflare tunnel and forwards `CRMBUILDER_V2_MCP_TOKEN` (same env file) as its bearer to the cloud API. A 401 on every tool while the connector's own login works means that token is missing — lesson LSN-081. Since 09-05-26 (REQ-567 / PI-469) the connector has read tools (list + get) for process, requirement, domain, persona, project and term records alongside the governance tools; the tool list is `crmbuilder-v2/src/crmbuilder_v2/mcp_server/tools.py`. Restarting the `crmbuilder-v2-mcp` service drops the selected engagement: call `select_engagement` again before any write, or the write fails with a 500 until PI-510 lands (LSN-084).
...
**Automatic at session start (PI-437 / REQ-540; two hooks since PI-488 / REQ-575, DEC-1062):** the `SessionStart` hook in `.claude/settings.json` runs `crmbuilder-v2/src/crmbuilder_v2/session_context.py`, which asks the store for the session opening (`GET /sessions/opening`): the cross-cutting rules — the one named set held by the system-scope profile whose area is `cross-cutting` (AGP-041) that loads whatever the phase — the active preferences, and the opening question *What do you want to do today?* with examples from the catalogue of kinds of work. A `UserPromptSubmit` hook runs `crmbuilder-v2/src/crmbuilder_v2/session_open.py`: the first user prompt is the opening answer (a pasted prompt file supplies a line beginning `Opening answer:`, else its first line); the hook calls the session-open operation (`POST /sessions/open`), which classifies the answer against the catalogue (process records PROC-012..PROC-020, trigger phrases in their triggers field), creates the session record (in flight, carrying the answer, the kind of work, the confirmation line and the phase segments), and returns the first segment's profile contract merged with the cross-cutting rules — placed in context before the model reads the prompt. The hook answers in Claude Code's JSON hook output: the contract goes to the model as context, and the line the user must be able to check — the confirmation line, the follow-up question, or the could-not-open notice — goes in `systemMessage`, which Claude Code displays; the contract also tells the model to begin its first reply with the confirmation line (PI-500 / REQ-594, REQ-595). Later prompts do nothing. An empty or unrecognised answer loads the cross-cutting rules only and says so; a miss returns one follow-up question and records a planning item. Per-session markers live in the gitignored `crmbuilder-v2/data/session-open/`. If the store is unreachable the start hook prints the last snapshot (`crmbuilder-v2/data/session-context.snapshot.md`, gitignored) under a visible banner, and the prompt hook leaves a visible notice naming the manual fallback (ask the question, call the connector tool `open_session`). An API without the opening endpoint falls back to the flat audience query. The manual reads below remain the fallback and cover steps 1, 4 and 5. Segments advance with `POST /sessions/{id}/advance-segment` (connector tool `advance_session_segment`).

**Enforced rules are checked before a command runs (PI-439 / REQ-542):** the `PreToolUse` hook runs `crmbuilder-v2/src/crmbuilder_v2/rule_check.py` on every Bash command and evaluates each enforced rule's check (`forbidden_command`, `required_trailer`, `protected_path`) against it. An `enforced` rule that fails blocks the command with the rule named; an `enforced_with_override` rule can be waved through by prefixing the command with `GVR_OVERRIDE='GVR-NNN: <reason>'`, and the reason is recorded in the store (`/governance-rules/{id}/enforcement-overrides`). Since PI-488 the enforced rules come first from the contract the session received (the per-session marker written by the two opening hooks); without a marker they are read from the store and cached for ten minutes in the gitignored `crmbuilder-v2/data/rule-checks.snapshot.json`; with no store and no snapshot the hook fails open with a warning.

**Records that contradict what is merged are reported, not corrected (PI-458 / REQ-556):** `uv run python -m crmbuilder_v2.record_drift` names any planning item still recorded as unstarted while its implementing commits are merged on `origin/main`, reading the `Governed-By` trailer the commit gate already requires. Exit 1 means a record is misleading and a person must move it — the check never writes, because closing an item rests on verifying acceptance criteria. It reads the store over the API and refuses if credentials are absent rather than falling back to the retired local SQLite. Details and flags: `crmbuilder-v2/README.md` §Planning-item record drift.

**A reference needs both of its records to exist (PI-502 / REQ-596..598, DEC-1081):** creating a reference refuses one that names a missing source or target record (`source_not_found` / `target_not_found`), before the row is written and so before any change the reference makes, such as a `resolves` edge marking its planning item Resolved. Six types with no string identifier of their own are exempt, listed once as `REFERENCE_EXISTENCE_EXEMPT_TYPES` in `crmbuilder-v2/src/crmbuilder_v2/access/vocab.py`. Write the record first, then the reference; in tests, `tests/crmbuilder_v2/_records.py` creates the minimal real record under a given identifier. `set -a; source crmbuilder-v2/data/crmbuilder.env; set +a; uv run python -m crmbuilder_v2.dangling_references` lists stored references that already point at a missing record, per engagement (exit 1 when it finds any); it never changes a reference. The seven the first census found were corrected by hand on 2026-09-14 (DEC-1085) and the check has exited 0 in every engagement since. A write that names no engagement still reaches the database and fails with a server error; refusing it before the database at the store (REQ-602) and at the connector (REQ-603) is PI-510, approved 2026-09-14 (DEC-1091) and unbuilt.

**Decisions can be recorded past DEC-999 (PI-493 / REQ-599, REQ-600):** the decisions repository accepts an explicit identifier of three or more digits, and the connector's create-decision tool leaves the identifier optional so the store assigns it. Rolled out to production on 2026-09-14 as commit `1c30431b` (code-only; store head unchanged at `0097`, publish check healthy).

**The deploy wizard supports manual DNS (PI-566 / REQ-642, DEC-1146):** a deploy run's `dns_mode` is `cloudflare` (default) or `manual`. Manual DNS takes the full address, needs no Cloudflare credential, shows the A record in the log and the progress status line, waits `CRMBUILDER_V2_MANUAL_DNS_WAIT_SECONDS` (default 1800) for public resolvers, and records `dns_provider = "manual"`. At Squarespace and most providers the record's host is only the first label (`crm`); typing the full address doubles the domain. Provider credentials are stored **per engagement** — the one the strip names when **Set credentials…** is used — so saving a chapter's token with the wrong engagement selected overwrites that engagement's (happened to ENG-002 on 2026-09-23, repaired). Rolled out to production on 2026-09-23 as commit `7b1a5062` (code-only).

**The engagement row is the application record (PI-575 / REQ-653, DEC-1155, DEC-1183):** an application is an engagement with a defining client and a visibility. The defining client is the client marked primary in the `engagement_clients` holding (no new column); `engagement_visibility` is `private` (default) or `public`. The access layer (`segments/client_management/repositories/engagement.py`) and the REST API read and write both on `/engagements`, unchanged for every existing caller, and the new `/applications` routes (list with `?client=`, get, create, replace, patch) serve the same rows under the model's name; creating or re-pointing to a missing or soft-deleted client is refused with `client_not_found` before any row is written, and an engagement no client holds is refused an application reading (`no_defining_client`) while still listing as an engagement. Connector read tool `list_applications`. ENG-003, ENG-006 and ENG-007 stay unassigned until the migration run PI-579 applies the approved mapping (DEC-1176..DEC-1182); the deployment record is PI-576, the purpose PI-577, the desktop and connector rename PI-580. Rolled out to production on 2026-09-25 as commit `10b447c2` (store head `client_management_0003_application_attributes`, publish check healthy).
...
2. Active **governance_rules** (`GET /governance-rules?resolution=effective`) — the binding operating rules migrated from this file, resolved for the active engagement: system defaults plus that engagement's overrides, an engagement rule shadowing the system rule of the same `rule_type` (REQ-529..533 / DEC-955). `?status=active` alone lists stored rows without the override resolution. Rules carry an audience (`applies_to`) and a moment (`applies_when`) (REQ-541) — the SessionStart hook already loads the session-audience set, so this read is its manual fallback. Authoring (REQ-543): a new rule names its `source_decision` (DEC-NNN); a text change declares `change: wording|meaning`; an enforced non-agent rule carries its check `predicate` (REQ-542). Details: `crmbuilder-v2/README.md` §MCP tools callouts, lessons LSN-063/LSN-065.
...
**Orientation is the DB.** The database is the single source of truth — see the **"Session bootstrap"** section above for the session-start read protocol (TOP-013 + active governance_rules / preferences / reference_pointers, lessons on demand). Do not orient from committed files or the local SQLite; read the store live.
...
**V2 audit has V1-audit parity (2026-08-30, PRJ-112).** The native audit's eleven areas (entities, fields incl. built-in fields, associations, all 18 layout types, roles, field permissions, teams, filtered tabs, email templates, field dynamic-logic rules, and the opt-in record-utilization profiler) cover everything the V1 Audit feature captures. Evidence: `PRDs/product/crmbuilder-v2/v1-audit-parity-gap-matrix.md` (the learn-phase inventory) and `PRDs/product/crmbuilder-v2/v1-audit-parity-live-diff.md` (the live V1-versus-V2 diff). V1 audit code was removed on 2026-09-01 (PI-454 / REQ-549); the reconcile feature's retained discovery/reverse core lives at `espo_impl/core/reconcile/capture.py`. Publish-side roles, teams and filtered tabs landed on 2026-09-03 (PI-417 / REQ-519 / REQ-521 / DEC-998: the entity-less `security.yaml` program, the `filteredTabs:` block, and the access-change confirmation gate). Publish-side layouts landed on 2026-09-04 (PI-427 emitter + PI-418 split, REQ-519/REQ-520): the adapter renders each entity's `layout:` block from confirmed layout records and the publish path applies it; portal- and role-bound variants are surfaced as non-actionable with the reason. With that, every audited area has a publish direction. Rolled out to production on 2026-09-04 as commit `48b6cf6e` (store head `0094_pi_418_portal_layout_types`, publish check healthy).

**Client, application and deployment (PRJ-128, 09-25-26).** The engagement row is read as the **application** (its defining client is the primary row in `engagement_clients`, its visibility a column; DEC-1183, PI-575). A **deployment** (`DPL-NNN`, system-wide table `deployments`, unique across applications) names its application, client, hosting provider and purpose (`client_own` | `demo_test`) and holds the instance, the instance deploy configuration and the provider credentials by composition (DEC-1185, DEC-1186; PI-576, PI-577): the three tables stay keyed by `(engagement, INST-NNN)`, so audit, publish, conformance and run history are unchanged, and `/instances` remains a view over the deployment. Provider credentials may be scoped to a deployment (`deployment_identifier`), the application-level row being the fallback. Code: `segments/operate/repositories/deployments.py`, `routers/deployments.py`, migrations `operate_0003`..`operate_0005`. The desktop shows Applications and Deployments panels and the connector has `get_application`, `list_deployments`, `get_deployment` (PI-580, DEC-1188). The approved mapping of the seven engagements is `PRDs/product/crmbuilder-v2/engagement-migration-mapping.md` (DEC-1176..DEC-1182); the migration run that applies it (`segments/operate/prj128_migration.py`, PI-579) ran in production on 09-25-26 under Doug's ruling on the Rochester comparison (DEC-1189, DEC-1190); the differences it accepted are listed in `prj128-migration-rehearsal-report.md` as candidate corrections to Cleveland's design. Production now holds four clients, four deployments (DPL-001..DPL-004, all of ENG-002) and archived ENG-003, ENG-005, ENG-006, ENG-007. PRJ-128 was accepted by Doug's walkthrough and marked complete on 09-26-26 (DEC-1191); the walkthrough and test plan are `prj128-test-plan.md`. Open beside it: Cleveland's application is private, so Rochester and Boston cannot add deployments through the desktop until it is made public or a per-client grant exists (DEC-1177).
...
**Two reference-creation forms (09-04-26, DEC-1042/DEC-1043, PI-463..465).** The References pane's New Reference dialog (`ui/dialogs/reference_create.py`) is the generic cascading form: it starts with nothing chosen and asks for both ends, and its identifier pickers list every record type by name (`_LIST_METHOD_NAMES` + `list_records_for_type`, REQ-562). A record's own detail view opens the record-side connection form instead (`ui/dialogs/record_connect.py`, REQ-563): the source is fixed and never shown, only the kinds allowed from that record's type are offered, eligible targets are listed with tick boxes, and one save creates one reference per tick; already-connected targets are locked. Both draw their lists from the same helper. The shared identifier picker (`ui/widgets/entity_identifier_picker.py`) opens as a compact popup with single-spaced rows and an in-list search box (REQ-564/REQ-565). Rolled out to production on 2026-09-05 as commit `59ef16ff` (code-only; store head unchanged at `0094`, publish check healthy).
...
**Detailed v2 build history lives in the database, not here.** The entity releases (identifier-optional POST, the `{data, meta, errors}` envelope, v0.7 governance entities, PI-073 session/conversation redesign, PI-112 governance/delivery model, PI-114 ADO substrate + its agent-layer evolution, the PI-122 Agent Profile Registry, the PI-330 registry UI, the reconcile redesigns, the PI-α/β/γ Postgres foundation, the requirements-provenance rebuild, …) are recorded as decisions, planning items, and lessons in the store — read them there (REL-039 SSoT). Enduring code-behaviour rules distilled from that history now live as `lesson` records (e.g. envelope-unwrap discipline, add-entity-type-needs-both-CHECKs, never-hand-patch-live-schema, dual-head Alembic) — `GET /lessons`.
...
The requirements-document methodology (the 13-phase Document Production Process for producing Master PRDs, Domain PRDs, Entity PRDs, process documents, YAML, and Verification Specs) is being consolidated into the Master CRMBuilder PRD and its methodology records live in the database as topics/requirements/decisions (per the "specs live in the DB" principle). The authoritative phase specification remains `PRDs/process/CRM-Builder-Document-Production-Process.docx` until the Master PRD subsumes it; the stakeholder-facing conduct rules are in `PRDs/process/conduct/` (see the CRMBuilder v2 section above). Read the methodology from the DB / that spec at the start of any requirements session; do not re-inline it here.
## Known Limitations
