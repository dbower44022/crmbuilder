...
UI: read-only **Workstreams** and **Work Tasks** monitoring panels under the Governance sidebar (`ui/panels/workstreams.py`, `work_tasks.py`). The closing work — the **agent prompts/skills** that wrap each substrate and the **runtime** that injects a contract and spawns an agent session — is deferred to the **Agent Profile Registry** (PRD at `PRDs/product/crmbuilder-v2/agent-profile-registry/`, v0.2; scoped as **PI-122**, decomposed and ready to build). Two tier prompts are already proven end-to-end and captured under `agent-profile-registry/profiles/` (Development Phase Specialist, Area Specialist). **Runtime note (from the agent proofs):** an agent's worktree must be spawned from current `main` HEAD, or it builds on stale code. **Agent-layer evolution in progress (06-01-26):** `PRDs/product/crmbuilder-v2/agent-delivery-organization-evolution.md` reshapes the agent layer (not the substrate) — a matrix org (passes × area-disciplines) of per-area Architect/Developer experts, phase-major + reconciliation, **standing *learning* experts** that write experience back to the V2 DB, cross-PI coordination, and a release-batched cadence; it materially expands what the registry (PI-122) must be. Read it before doing further ADO agent-layer or registry design.
