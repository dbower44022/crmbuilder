...
- **Tier 2 (v2 engagement) — MCP or REST API, the two equal options:** The database is the single source of truth; read it live, never from committed files (PI-β removed the `db-export/` JSON snapshots). When the `crmbuilder-v2` MCP server is connected, call `get_current_status`, `get_current_charter`, `list_recent_sessions(limit=3)`, then `get_decision(<id>)` / `list_decisions_for_session(<id>)` as referenced. Otherwise hit the REST API directly (`curl http://127.0.0.1:8765/status`, `/charter`, `/sessions?...`, `/decisions/<id>`, unwrapping the `{data, meta, errors}` envelope), naming the engagement with the `X-Engagement` header. Start the server with `crmbuilder-v2-api &` (MCP additionally needs `crmbuilder-v2-mcp`).
