...
**Agent Delivery Organization (ADO) substrate landed — PI-114 / WTK-001…006 (05-31-26).** The runtime organization PI-112 deferred is now built as a **deterministic REST/access substrate** (the LLM-judgment agents that drive it, and the agent-profile registry that holds their prompts, are the remaining §10 follow-on — see below). Design: `PRDs/product/crmbuilder-v2/agent-delivery-organization-design.md` (v0.3). The four agent tiers each got a substrate, all reconstructable from the records (DB-backed statelessness, §4.4):
