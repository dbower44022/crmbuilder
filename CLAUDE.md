...
- **Local API** `http://127.0.0.1:8765` for local dev (start with `crmbuilder-v2-api &`). **Dev-only:** its SQLite store was retired as a source of truth by the 07-01 cloud cutover — it is stale and may sit behind on migrations (the API refuses to start until `crmbuilder-v2-bootstrap-db` is run). Never read or record governance state against it.
...
**Storage system v0.1 has landed.** Charter, status, decisions, and sessions live in the v2 database (gitignored). The four governance markdown files that previously held this content have been retired (recoverable through git history). The PRD and the implementation plan remain in markdown at `PRDs/product/crmbuilder-v2/` because they are external specs, not bootstrapped governance content. (Historical note: v0.1–PI-α also wrote git-tracked JSON snapshots at `PRDs/product/crmbuilder-v2/db-export/`; **PI-β removed that snapshot tree** — the DB is the source of truth and the git-tracked governance trail is the close-out payloads + deposit-event logs. See the PI-β note below.)
...
- **Tier 2 (v2 engagement) — MCP or REST API, the two equal options:** The database is the single source of truth; read it live, never from committed files (PI-β removed the `db-export/` JSON snapshots). When the `crmbuilder-v2` MCP server is connected, call `get_current_status`, `get_current_charter`, `list_recent_sessions(limit=3)`, then `get_decision(<id>)` / `list_decisions_for_session(<id>)` as referenced. Otherwise hit the **cloud REST API** directly (`curl -H "Authorization: Bearer <token>" -H "X-Engagement: ENG-001" https://api.crmbuilder.ai/status`, `/charter`, `/sessions?...`, `/decisions/<id>`, unwrapping the `{data, meta, errors}` envelope) — base URL + token from `crmbuilder-v2/data/crmbuilder.env` per "Session bootstrap" above. Do NOT orient from the local API/SQLite (retired as a source of truth, 07-01 cloud cutover); it exists for local dev only.
...
**Requirements-provenance & review rebuild — COMPLETE (06-13-26).** A V2 process rebuild so a capability can never be built without a traced, human-reviewable requirement (trigger: the agent-rules capability shipped with no requirement record). Two PRs, both merged to `main`: **PR #4** (the engine, Phases 1–6) and **PR #5** (the Qt review panel, the coverage baseline cutoff, and the Phase 7 proof-on-itself). Governing design: `PRDs/product/crmbuilder-v2/requirements-provenance-and-review-anchor.md` + `...-build-translation.md`; Phase 7 record: `...-phase7-prove-on-itself.md`. What landed:
- **Schema/edges (migrations 0049–0051):** requirement columns `requirement_origin {human_defined,ai_derived}` + `requirement_review_state {current,needs_review}` + `requirement_approved_at`; six provenance edge kinds — `requirement_refines_requirement`, `requirement_defined_in_conversation`, `requirement_belongs_to_topic`, `conversation_belongs_to_topic`, `requirement_approved_by_decision`, `requirement_changed_by_decision` — plus `planning_item_implements_requirement` (no-orphan-capability) and an append-only `review_signoffs` entity.
- **Gates (access layer):** a requirement confirms only via an approving decision edge **and** only when its provenance + topic resolve, directly or through `requirement_refines_requirement` ancestry; a **blocking readability gate** (`access/readability.py`) rejects embedded identifiers / unreadable statements at approval; **living drift** flags descendants `needs_review` when a parent's meaning changes.
- **Surfaces:** `/review/topics/{id}` (tree), `/review/topics/{id}/document` (read-back), `/review/approval-queue`, `/review/drift-queue`, `/review/signoffs`, and `/coverage/capabilities` (the no-orphan report). The desktop **Requirements Review** panel (`ui/panels/review.py`, Governance sidebar) renders all of it and is the only review write (the sign-off). Record creation is otherwise API/MCP-only.
- **Coverage baseline cutoff (durable):** `/coverage/capabilities?since=<iso>` (or the `CRMBUILDER_V2_PROVENANCE_BASELINE` default) splits legacy pre-model debt (`baseline_summary`) from live gaps, so the report stops drowning in the pre-provenance corpus. The default is set durably in the gitignored, repo-rooted **`crmbuilder-v2/data/crmbuilder.env`** — read by `Settings` on every process start (standalone or UI-spawned), real env vars still override, tests forced hermetic via the `v2_env` fixture. This `.env` is now the home for any durable `CRMBUILDER_V2_*` default. **ENG-001's durable baseline is `2026-06-10`** — recorded here because that `.env` is gitignored/per-deployment and won't travel with a clone. The line was moved from 06-13 to 06-10 (DEC-417) to bring the 06-11/12 build session's 17 then-orphan capabilities (`PI-145..161`) into live scope rather than grandfather them as pre-model debt; they were then secured under six confirmed requirements `REQ-115..120` (`SES-170`, provenance conversations `CNV-072..076`), so coverage holds `orphan_planning_items: 0` at the 06-10 line while genuinely older work (133 items) stays excused as legacy baseline debt.
- **Phase 7 (proven on itself, against ENG-001):** the engine cleanly holds its own founding requirement — `TOP-087` (topic) ← `SES-169` ← `CNV-071` (provenance root) ← `DEC-415` (approving decision); founding requirement `REQ-108` (human_defined, confirmed) + six child rules `REQ-109..114`; plan links `PI-162..167` (coverage `orphan_planning_items: 0`); sign-off recorded. The live `v2-unified.db` is migrated to head **`0051`** (pre-migration backup at `crmbuilder-v2/data/v2-unified.db.bak-pre0051`).
- **Coverage edge backfill (ENG-001, 06-13-26):** recovered real provenance for the legacy corpus rather than fabricating it. The 101 confirmed-but-"unbuilt" requirements were clustered by the design **decision** each traces to (they predate topics/provenance edges), and each decision-cluster linked to the Resolved PI(s) that built it via `planning_item_implements_requirement` (e.g. DEC-385 four-pass model → PI-112/114, DEC-388 runtime → PI-132/139/143, DEC-382/381 learning+registry → PI-122, governance-recording rules → PI-130, session/conversation redesign → PI-073). ~138 edges, **`unbuilt_requirements` 101 → 6** and **`orphan_planning_items` 160 → 152**. Two no-decision agent-system reqs were linked by hand (`REQ-003` → PI-123 unified-DB/engagements; `REQ-014` "all agent work documented and visible" → PI-114 + PI-120 the Workstream monitoring panel). The 6 holdouts are genuinely unbuilt (DEC-300 tooling, the DEC-025 *deferral*, and the `REQ-108` umbrella). Done as direct API writes (DEC-383), no git artifact. The lesson recorded for future legacy backfills: cluster by decision/conversation provenance and map to the implementing PI — never fuzzy-match requirement names to PI titles (the two sets are disjoint domains; that fabricates provenance).
...
**Governance is a precondition, not a postscript (read before any V2 build).** Before any development work begins on a V2 capability — code, schema, migration, or build of any kind, **on any branch** — the following must already exist, **in this order**:

1. a **requirement** record stating the capability and why, with provenance (defined in a conversation, belonging to a topic);
2. that requirement **confirmed** through the approving-decision path (`requirement_approved_by_decision` edge → `activate_by_decision`, which stamps `requirement_approved_at`) — i.e. **human-reviewed and approved**, *never* by editing the status field;
3. a **planning item** that implements the requirement (`planning_item_implements_requirement` edge), inside a **project**.

Only then is code written. There is **no "too small for a requirement" exception** — if a change is genuinely trivial and below the requirement threshold, that judgment must itself be stated, not silently assumed. Building before a confirmed requirement + implementing PI exist is a process violation even on a branch and even for a one-line change. This is ENG-001's founding rule (*"a capability can never be built without a traced, human-reviewable requirement"* — see "Requirements-provenance & review rebuild" below) made an explicit, binding precondition.

Distinguish two different "whens" — conflating them is the exact failure mode this rule closes:

- **WHEN the requirement must exist** — *before any code.* The requirement, its human approval, and the implementing PI are the authorization to build and are **not deferrable**.
- **WHEN the governance *bookkeeping* lands** — the session record, decisions, and deposit-event log are deferred to `main` after merge by the **Branch-work protocol (Model A)** below. That protocol defers the *bookkeeping only* — it does **not** defer the requirement, its approval, or the implementing PI.

**Self-check (the one most often skipped):** before writing or committing any code, confirm out loud that the requirement is confirmed and the implementing PI exists. If not, **stop and create them first.** The canonical governance rules for this live in the V2 DB under topic `TOP-013` (see "v2 governance recording rules" below).

**Governance enforcement gate — automatic (REQ-320 / PI-286/287, 06-30-26).** The precondition above is now enforced by a git hook, not just discipline. Every commit/push **touching code** must carry a **`Governed-By: PI-NNN`** trailer naming a PI that is in an executable state, belongs to a project, and implements a **confirmed** requirement — validated *live* against the governance store (`crmbuilder_v2.governance_gate`). A genuinely trivial change instead carries **`Governed-By: trivial`** + a non-empty **`Exemption-Reason: <why>`** (logged to `PRDs/product/crmbuilder-v2/governance-exemptions.log`); an empty reason is rejected. **Doc/data/governance-only commits** (`PRDs/`, `*.md`, `crmbuilder-v2/data/`, …) are auto-exempt — no trailer needed. **Mode** via `CRMBUILDER_GOVERNANCE_GATE` = `off | warn | enforce` (**default `warn`** during rollout — surfaces violations, blocks nothing; flip to `enforce` once warnings are clean). **Install once per clone:** `uv run crmbuilder-v2-install-hooks` (sets `core.hooksPath=crmbuilder-v2/githooks`; worktrees inherit it, covering the ADO fleet; coexists with the existing deposit-event `pre-commit` guard). Design: `PRDs/product/crmbuilder-v2/governance-enforcement-gate-design.md`. **Add the trailer to every code commit** (humans and agents) — the gate dogfoods on itself.

**Retain-not-delete: cleaning up a failed run preserves its record (REQ-264, PRJ-065, 06-27-26).** Cleaning up after a failed or abandoned release run means **retiring it, never erasing it.** No cleanup or teardown operation may hard-delete the evidence of what a run scoped, which phases ran, or where it failed — binding on humans and AI agents alike. The **only sanctioned cleanup path** for a release that has entered a lane is **`releases.abandon()`** (`POST /releases/{id}/abandon`), which writes a born-terminal `release_run` outcome record (scope snapshot, per-phase terminal states, halt point, cause) and **preserves** the `project_belongs_to_release` scope edges and the phase workstreams as the run's evidence — a plain lane→cancelled transition is refused, and the cancel-time `_clear_decompositions` is bypassed under abandon. The scope-membership guard counts only **active** releases (`RELEASE_TERMINAL_STATUSES = {cancelled, superseded, shipped}`), so an abandoned run keeps its composition while the project re-scopes into a new run; the Releases panel's Composition + Outcome tabs surface it. Recorded as `governance_rule` **GVR-122** (enforced, bound to the Release Lead profile `AGP-005`); the run-outcome record is `release_run` (`RUN-NNN`).
...
**Branch-work protocol (Model A).** *(This protocol governs WHEN governance bookkeeping lands — never WHEN the requirement must exist. The requirement, its human approval, and the implementing PI must already exist before any branch work starts; see "Governance is a precondition, not a postscript" above. Do not read the deferral below as license to build first and record governance later.)* Governance applies happen only on `main`. A `pi-NNN` branch carries only code, schema, and migration commits — it never runs `apply_close_out.py` and never commits anything under `PRDs/product/crmbuilder-v2/deposit-event-logs/`. The branch ships the migration; the work's sessions, decisions, and planning items are authored as a close-out payload and applied on `main` after merge, re-keyed to `main`'s current heads (the DEC-232 / SES-074 build-closure pattern). This keeps the governance identifier sequence and the committed deposit-event logs advancing on a single line; two lines advancing them independently produces duplicate identifiers. Enforced mechanically: `apply_close_out.py` refuses to run off `main` (override `--allow-branch-local` for Model B isolated-DB work, which also requires `CRMBUILDER_V2_DATABASE_URL`/`CRMBUILDER_V2_DB_PATH` set to a gitignored branch-local store), and the `crmbuilder-v2/githooks/pre-commit` hook rejects staged `deposit-event-logs/` commits off `main` (merge-time override `CRMBUILDER_ALLOW_BRANCH_SNAPSHOT=1`). The PI-073 isolated-DB note remains the worked example of the *capability*; this paragraph is the rule. (PI-β removed the `db-export/` JSON snapshot tree, so the deposit-event logs are now the only git-tracked governance trail the hook guards.)

**v2 session lifecycle — opening a session.** Each unit of work in v2 is anchored by a `planning_item` (`PI-NNN`) in the V2 governance DB, with an associated `work_ticket` of kind `kickoff_prompt` (`WT-NNN`) that carries the kickoff body summary plus a `work_ticket_file_path` pointer to the canonical kickoff `.md` in `PRDs/product/crmbuilder-v2/`. The WT has an `addresses` reference edge to the PI. To open a session: `GET /work-tickets/WT-NNN` against the cloud API (see "Session bootstrap" for base URL + auth; or `get_work_ticket("WT-NNN")` if MCP-connected) → read the description summary for orientation → fetch the file at `work_ticket_file_path` for the full kickoff (Tier 1–5 reads, goals, open questions, deliverable shape). Alternatively, *"Open against PI-NNN"* triggers the session to query `GET /references?target_id=PI-NNN&relationship=addresses` first, find the addressing WT, then proceed identically. The kickoff is the source of truth for what the session does; the PI is the source of truth for *why* and what artifact ultimately resolves it. **Scheduled-session handoff (PI-073 + ADO):** a session may instead be *pre-created* in `planned` status to hand work to the next session — the planned session's `session_description` carries the kickoff, wired via `session_belongs_to_project` and (for a Work Task) `session_works_work_task` → `WTK-NNN`. To open it, fetch the planned `SES-NNN`, transition it `planned → in_flight`, and execute. See the Session Records rules under V2 topic `TOP-079` (governance recording rules now live in V2 under `TOP-013` — see the "v2 governance recording rules" note below) and `PRDs/product/NEW-Master PRDs/Agent PRDs/Archive/agent-delivery-organization-design.md` §3.5.

**v2 session lifecycle — closing a session.** Every session produces a triple-artifact close-out: (1) a content deliverable (design doc, code commits + tests, methodology amendment, etc.) committed under `PRDs/product/crmbuilder-v2/` or the appropriate code path; (2) a close-out payload at `PRDs/product/crmbuilder-v2/close-out-payloads/ses_NNN.json` (nine sections per v0.8: `session`, `conversation`, `work_tickets`, `planning_items`, `commits`, `decisions`, `references`, `resolves_planning_items`, `addresses_planning_items` — empty sections are still listed); (3) an apply prompt at `PRDs/product/crmbuilder-v2/prompts/CLAUDE-CODE-PROMPT-apply-close-out-ses-NNN.md` that documents the pre-flight checks, any workstream pre-steps, the apply command, and post-apply verification. The session runs `uv run python scripts/apply_close_out.py ../PRDs/product/crmbuilder-v2/close-out-payloads/ses_NNN.json`, which atomically writes all records and lazy-creates the `close_out_payload` + `deposit_event` entities. The session then commits the new `deposit-event-logs/dep_NNN.log` + the content deliverable + payload + apply prompt in one commit. (PI-β removed the `db-export/*.json` snapshot regeneration — there is no snapshot tree to commit; the DB is the source of truth and the deposit-event log is the git-tracked trail.) SES-074 and SES-077 are the most recent worked examples.
...
**v2 governance recording rules.** Every session and conversation operating against any V2-tracked engagement follows the canonical governance recording rules, which now **live in V2 under topic `TOP-013` — "Governance Recording Method"** (migrated out of the retired file `specifications/governance-recording-rules.md` by PI-130 on 06-04-26, per DEC-393/DEC-394). Each rule is a `requirement` record under TOP-013 or one of its child topics (TOP-076 Core Recording Principles, TOP-077 Identifier Discipline, TOP-078 Project Records, TOP-079 Session Records, TOP-080 Conversation Records, TOP-081 Decision Records, TOP-082 Planning Item Records, TOP-083 Reference Records, TOP-084 Work Ticket Records, TOP-085 Recording Mechanism, TOP-086 Wire-Format Constraints), stated in plain declarative language and traced to its source decision via a `references` edge. Applies equally to AI agents (Claude.ai sandbox conversations, Claude Code instances) and human agents (DEC-310). **Read TOP-013 before authoring any governance record** — `GET /topics/TOP-013` against the cloud API (base URL + auth per "Session bootstrap"; `X-Engagement` header names the engagement), then `GET /references?target_id=TOP-0NN&relationship=is_about` per child topic, or browse the Topics / Requirements panels. **Real-time recording (DEC-383, 06-03-26):** in Claude Code, where the live API is reachable, governance records are entered in real time via direct API POST at the moment they occur (`POST /decisions`, `/planning-items`, `/sessions`, `/conversations`, `/references`) — **not** batched into a close-out JSON payload. The close-out-payload + `apply_close_out.py` path is the **fallback for the claude.ai sandbox** (no API access); in Claude Code it retains only the residual role of producing the git-tracked deposit-event log under the Model A branch protocol, and there is no API-level block on direct POSTs. The session-lifecycle bullets above are summary entry points; V2 topic `TOP-013` is the authoritative reference.
...
- **The word "Workstream" was reused for a NEW entity** (DEC-343/DEC-349): a single **delivery phase** of one Planning Item (identifier **`WSK-`**, table `workstreams`, `/workstreams`). Phase-type vocab `{Architecture, Development, Testing, Documentation, Data Migration, Deployment}`; lifecycle `Planned → Scoping → Ready → In Progress → Complete | Not Applicable | Blocked`. Belongs to a Planning Item via `workstream_belongs_to_planning_item`. **ADO state-model substrate landed (WTK-001 / migration `0036`, DEC-349 rename + DEC-359 flag):** the phase value `Design` was renamed `Architecture`; the lifecycle was expanded from the original `Planned → In Progress → Complete (+Blocked)` to the gate model above (mirrors the Work Task's `Ready/Claimed/In Progress` for unambiguous Lead gate signals); and an orthogonal human-escape flag `workstream_needs_attention` (bool) + `workstream_needs_attention_reason` (text) overlays the status without erasing progress (set by Lead/PM, cleared by a human; rollup to the Planning Item is derived at query time, not a stored PI column).
...
- **Deferred (not done by PI-112):** dropping the now-redundant `planning_item.area` column — its only reader is the shelved WS-012 orchestrator (DEC-344), so it belongs to that orchestrator's retirement (target-model §9 step 6). The runtime **agent-delivery organization** that consumes these entities (DEC-343) was PI-112's explicit downstream successor — **its substrate has since landed (PI-114; see the "Agent Delivery Organization" note below)**, and the Workstream/Work Task **UI monitoring panels are now built** (no longer deferred). Engagement-area management UI remains a deferred follow-on.

**Agent Delivery Organization (ADO) substrate landed — PI-114 / WTK-001…006 (05-31-26).** The runtime organization PI-112 deferred is now built as a **deterministic REST/access substrate** (the LLM-judgment agents that drive it, and the agent-profile registry that holds their prompts, are the remaining §10 follow-on — see below). Design: `PRDs/product/NEW-Master PRDs/Agent PRDs/Archive/agent-delivery-organization-design.md` (v0.3). **(Doc-location note, 06-20-26:** the agent/orchestrator/ADO + release-pipeline + agent-profile-registry design/build docs were consolidated into `PRDs/product/NEW-Master PRDs/Agent PRDs/Archive/` — see its `README.md` manifest — pending a consolidation pass that produces a clean overview + technical reference. Paths in this file point to the archive.) The four agent tiers each got a substrate, all reconstructable from the records (DB-backed statelessness, §4.4):

- **Project Manager** (tier 1) — `access/repositories/pm.py`: `project_backlog` (a Project's PIs with `blocked_by` dependency-eligibility + the eligible/in_flight/blocked/resolved partitions), `eligible_planning_items`, `dispatch_planning_item` (eligible PI → In Progress). Endpoints `GET /projects/{id}/backlog`, `GET /projects/{id}/eligible-planning-items`, `POST /planning-items/{id}/dispatch`.
- **PI Lead** (tier 2) — `access/repositories/lead.py`: `phase_overview` (gate state — every phase Workstream with status, Work Task progress, serial `blocked_by` readiness, and the `all_scoped`/`all_terminal`/`needs_attention`/`next_executable` flags), `start_phase` (Ready → In Progress + ready its Work Tasks, gated on predecessors terminal), `complete_phase` (verify all Work Tasks Complete → In Progress → Complete). Endpoints `GET /planning-items/{id}/phase-overview`, `POST /workstreams/{id}/start-execution`, `POST /workstreams/{id}/complete-phase`.
- **Phase Specialist** (tier 3) — the **structural decomposer** `access/repositories/decomposition.py` (`POST /planning-items/{id}/decompose` → all six phase Workstreams `Planned` + serial `blocked_by` chain, once-only) plus the **scoping substrate** `access/repositories/scoping.py`: `scope_workstream` (`POST /workstreams/{id}/scope` — create the phase's Work Tasks + drive Planned → Scoping → Ready, or → Not Applicable for an empty scope) and `prior_phase_outputs` (`GET /workstreams/{id}/prior-phase-outputs` — the feed-forward read of earlier phases' Work Tasks).
- **Area Specialist** (tier 4) — does the single-area implementation work for one Work Task; its substrate is the existing Work Task claim/lifecycle endpoints.

UI: read-only **Workstreams** and **Work Tasks** monitoring panels under the Governance sidebar (`ui/panels/workstreams.py`, `work_tasks.py`). The closing work — the **agent prompts/skills** that wrap each substrate and the **runtime** that injects a contract and spawns an agent session — is the **Agent Profile Registry** (PRD at `PRDs/product/NEW-Master PRDs/Agent PRDs/Archive/agent-profile-registry/`; scoped as **PI-122**). Two tier prompts are already proven end-to-end and captured under `PRDs/product/NEW-Master PRDs/Agent PRDs/Archive/agent-profile-registry/profiles/` (Development Phase Specialist, Area Specialist — these are the *old-taxonomy* names; under the evolution below they map to a Development-area Architect-tier and a Developer-tier profile). **Runtime note (from the agent proofs):** an agent's worktree must be spawned from current `main` HEAD, or it builds on stale code.

**ADO agent-layer evolution — DESIGN COMPLETE + GOVERNED (06-01-26).** `PRDs/product/NEW-Master PRDs/Agent PRDs/Archive/agent-delivery-organization-evolution.md` (v0.3, design-complete) reshaped the agent layer (**not** the substrate, which is unchanged). It is now **governed as DEC-367…374** (recorded via **SES-149** and **SES-150**) and **execution-planned** — its §10 NEXT list is fully closed. The model: a **matrix org** (four passes Plan/Design/Develop/Test × area-disciplines) of per-area experts split into **three tiers — Architect / Developer / Tester** (DEC-368; build areas get all three, design/methodology areas Architect-only); **phase-major with a reconciliation gate** backed by a new **`finding` (`FND-`)** entity (DEC-367); **standing *learning* experts** that write experience back to the V2 DB via a new **`learning` (`LRN-`)** entity + a graded conservative-then-earned promotion gate (DEC-369); **cross-PI coordination** (the horizontal axis) with two-axis reconciliation; a **release-batched** default cadence with a coarse all-or-nothing design gate and a new **`REL-`** entity (DEC-370/371); a **spawn-on-demand runtime** where "standing" = contract scope, not a process (DEC-372). This **materially expanded the registry (PI-122)** — now the **Agent Profile Registry PRD v0.3** (new §13/§14): a *living, system-level learning knowledge base*, not a config catalog, with a **`system | engagement` scope discriminator** on every row + cross-engagement learning (DEC-373). **Sequencing:** the registry's cross-engagement learning needs a single multi-tenant DB, so the **unified multi-engagement-DB migration is scoped as PI-123** (Draft) under a new **Production Database Architecture project PRJ-019**, with **PI-122 `blocked_by` PI-123** (DEC-374). Build order (registry PRD §14): **PI-123 → PI-122 → runtime scheduler**. **PI-123's next session is its Architecture/design pass** — produce the unified-DB migration design (a design doc + decomposition) *before* any migration code; its crux is the **cross-engagement identifier collision** (CRMBUILDER's `SES-150` vs CBM's `SES-150` in one DB → composite `(engagement_id, identifier)` keying to preserve history) plus the access-layer routing rewrite (DB-path selection → row-scoped `engagement_id` filter). Read the evolution doc + registry PRD v0.3 before doing further ADO agent-layer or registry work.

**Agent Profile Registry substrate landed — PI-122 (06-03-26, on branch `pi-122-registry`, pending merge + build-closure).** The registry that *holds, resolves, and now grows* the ADO agents' skills/rules/learnings is built as a REST/access/MCP substrate against the now-landed PI-123 unified DB + PI-γ principals (Architecture: `PRDs/product/NEW-Master PRDs/Agent PRDs/Archive/pi-122-agent-profile-registry-architecture.md`; PRD: `PRDs/product/NEW-Master PRDs/Agent PRDs/Archive/agent-profile-registry/agent-profile-registry-PRD-v0.1.md` v0.3). **Four governance entities**, all **system/shared with a NULLABLE `engagement_id` scope** (NULL=system row, set=engagement overlay) — plain `Base`, **not** `EngagementScopedMixin` (the resolver does the scope merge explicitly so system rows stay visible; mirrors PI-γ's `role_assignments`): `agent_profile` (`AGP-`, keyed to an (area × tier) cell), `skill` (`SKL-`, instruction|tool + io_contract), `governance_rule` (`GVR-`, hybrid advisory|enforced|enforced_with_override), `learning` (`LRN-`, evidence-tagged, confidence). **Binding/learning edges** in the reference vocab: `agent_profile_has_skill`, `agent_profile_governed_by_rule`, `learning_derived_from`/`_contradicted_by`/`_promoted_to` (the `→ finding` target waits on the `finding` entity — an ADO-reconciliation build concern). **Resolver** (`access/repositories/registry_resolver.py`): `GET /agent-profiles/{id}/contract` composes an effective contract (system_prompt + tools + enforced_ruleset + active (area,tier) learnings + a deterministic version_stamp) with the scope merge `WHERE engagement_id IS NULL OR == active`. **Write-back lifecycle** (`registry_lifecycle.py`): capture-at-Work-Task-close → accumulate (evidence raises/contradiction lowers confidence) → promote (to skill = float/pin; to **enforced rule = human review always**, the Needs-Attention hard line) → per-(release,area) `curate` sweep; **cross-engagement promotion** (`cross-engagement-candidates` + `promote-to-system`) is the DEC-373 multiplier. **MCP tools** expose resolve-contract / list / create / capture-learning. **An `agent_profile` is the template; a PI-γ `service_agent` principal (with its `agent_tier`/`agent_area`) is the runtime instance** — the runtime scheduler (the separate §10 follow-on) resolves a contract + `mint_agent_principal`s the agent. SQLite migrations `0043`/`0044`, PG `0005`/`0006` (create the 4 tables + rebuild change_log/refs entity-type + relationship_kind CHECKs from current vocab, guarded). **Deferred follow-ons:** ~~the read-only Qt monitoring panels for the 4 entities~~ (superseded — see PI-330 below); the resolver's engagement override/disable of a system rule (since built, WTK-001); the runtime scheduler (largely built — the ADO scheduler resolves a profile via `dispatcher.select_profile_id` and injects its contract into the spawned agent via `scheduler/agent_prompt.build_agent_prompt`; per-agent `mint_agent_principal` minting at spawn is now built too — gated on `principal_auth_enabled`, see PI-340 below). Build-closure (records PI-122 governance, resolves PI-122) lands on `main` after merge.

**Registry contract → runtime: tools + provenance wired (REQ-380 / PI-339 / DEC-770, REL-029 / PRJ-069, 06-27-26).** `scheduler/agent_prompt.build_agent_prompt` already injected a resolved profile's system_prompt + enforced ruleset + active learnings into a spawned agent; PI-339 closes two verified gaps: (1) it now also renders a **"Tools available to you"** section from `contract["tools"]` (each tool's name · description · `backing_callable` · `io_contract`), so the registry's *tool* bindings actually reach the agent — and `registry_resolver` now includes `description` in each contract tool to support it; (2) the resolved contract's **`version_stamp`** is threaded through `_ResolvedAssignment` and recorded on the **dispatch pipeline event** in both the serial (`coordinating_scheduler.run_one`) and parallel (`parallel_scheduler._fill_slots` — which previously emitted no dispatch event) paths, so a run traces to the exact contract that drove it. Tests: `tests/crmbuilder_v2/scheduler/test_agent_prompt.py`. (Agent search has since shipped — see PI-343 below.)

**Agent search — endpoint + UI (REQ-383 / PI-343 / DEC-776, REL-029 / PRJ-069, 06-27-26).** Exposes the deterministic `agent_profiles.search_agents` pre-filter (PI-301/DEC-677), which had no REST route. New **`GET /agent-profiles/search`** (placed before `/{identifier}`): `area` required (the hard backstop, never crossed), optional `technology`/`tier`, `needs` (comma-separated capability hints, ranked by overlap with each profile's `capability_description` facets), `status` default `active`, scope from the active engagement. New `client.search_agents(...)` and a **"Find agents…"** dialog on the Agent Profiles panel (area/technology/tier/needs inputs → ranked results → open-in-panel via `select_record_by_identifier`). Tests: `tests/crmbuilder_v2/api/test_registry_resolver.py` (search cases) + `tests/crmbuilder_v2/ui/test_agent_registry_panels.py`. Note: `technology` is not settable via the create API (not in `AgentProfileCreateIn`), so the technology filter is covered by the repo's own tests. **The agent-registry surface is now complete** (config UI + contract-driven spawning + identity + search).

**Real per-area agent contracts seeded (REQ-386 / PI-346 / DEC-780, REL-029 / PRJ-069, 06-27-26).** The roster was complete but most profiles shared a generic per-archetype prompt templated with `{AREA}`. This populates the registry with **real, codebase-specific content**: a new `access/repositories/registry_seed_content.py` (`area_profile_content()` — the single source of truth) composes a per-tier operational skeleton with a real per-area domain block (drawn from these CLAUDE.md conventions) for every (area × tier) cell, plus the area's own governance rules (`rule_type`'d) and a searchable `capability_description`. `registry_seed.py` consumes it in `_catalog_profiles` (overriding the generic prompt, appending area rules, setting capability_description on create; seed stays create-only idempotent) so **a fresh DB seeds real contracts**, and the **live DB was patched from the same function** (38 profiles → 38 distinct prompts, all with capability descriptions; ~5 area rules each). model/planning/release keep their bespoke prompts and gain only a capability description. The live content is registry *data* (gitignored DB); the durable source is the seed code. Tests in `tests/crmbuilder_v2/access/test_registry_seed_catalog.py`.

**Per-agent principal minting at spawn (REQ-381 / PI-340 / DEC-771, REL-029 / PRJ-069, 06-27-26).** **Gated on `Settings.principal_auth_enabled` (default OFF — the localhost flow is unchanged).** When auth is on, `scheduler/agent_identity.mint_for_spawn` mints each spawned agent its own `service_agent` principal + bearer token (direct `session_scope`, the `event_capture` pattern — no orchestrator bootstrap token needed; `role="area_specialist"`, `agent_tier`/`agent_area` set); the token + principal_id thread through `_ResolvedAssignment`; `operating_protocol` injects `Authorization: Bearer …` into the agent's curls and sets the claim's `claimed_by` to the minted `principal_id` (so `enforce_claim_identity` passes); the token is **revoked after the run** in a `finally` in both the serial (`run_one`) and parallel (`_worker`) paths. Tests: `tests/crmbuilder_v2/scheduler/test_agent_identity.py`. **Companion gap — now closed by PI-341 (see below):** the scheduler's *own* API calls authenticate too.

**Scheduler self-authentication (REQ-382 / PI-341 / DEC-775, REL-029 / PRJ-069, 06-27-26).** Completes auth-on usability: the scheduler's own REST calls now carry the orchestrator's bearer token when auth is on. New `Settings.orchestrator_token` (env `CRMBUILDER_V2_ORCHESTRATOR_TOKEN`, default `""`, mirrors `mcp_token`) and a shared `scheduler/runtime_auth.auth_headers(engagement, content_type)` helper that adds `Authorization: Bearer <token>` when configured (mirrors `mcp_server/server.py`). Wired into all six scheduler HTTP sites: `dispatcher._get/_post/_patch`, `agent_prompt._get`, and the two `ado_scheduler` PI-status PATCHes. Direct-DB side-band writes (`event_capture`/`cost_capture`/`agent_identity`) bypass HTTP and are untouched. With per-agent identity (PI-340), flipping `principal_auth_enabled` on is now usable end-to-end — the orchestrator authenticates as its own principal and agents as themselves. **Bootstrap (ops, not built):** mint an orchestrator principal + token once with sufficient permissions and set the env var. Tests: `tests/crmbuilder_v2/scheduler/test_runtime_auth.py`.

**Configurable Agent Registry UI — PI-330 (REL-026 / REQ-367, 06-26-26, on branch `pi-330-agent-registry-ui`, pending merge + build-closure).** Replaces the deferred *read-only* panel with a **fully configurable** desktop surface over the PI-122 substrate (no new backend — all four entities already had REST CRUD + resolver + binding edges + learning lifecycle). New **"Agent Registry" sidebar group** (between Release Pipeline and Methodology) with four panels: **Agent Profiles** (`ui/panels/agent_profiles.py` — the centrepiece: profile CRUD, bound-skills/rules add+remove via the `agent_profile_has_skill`/`agent_profile_governed_by_rule` edges, and a live **effective-contract preview** with an engagement selector that re-resolves system-defaults-vs-overlay), **Skills**, **Governance Rules**, **Learnings** (`registry_skills.py`/`registry_rules.py`/`registry_learnings.py` over the shared `_registry_panel_base.RegistryCrudPanel`). Every row shows `scope` (system vs engagement); create dialogs build the scope combo from the live engagement list so an operator authors a system default or an engagement overlay (override = same `rule_type`; disable = `disable:<id>` rule_type). Scope is **editable on edit too** (DEC-750) — a record can be re-scoped between system and an engagement; the access-layer `.update()` maps `scope` → `engagement_id` for all four entities. JSON columns (`capability_description`/`io_contract`/`predicate`) edit through `JsonFieldDialog`; learnings promote to skill/rule (enforced-rule promotion gated on an explicit human-approval check). **Learning evidence + confidence (PI-336 / DEC-762, also implements REQ-367):** the Learnings panel additionally has an "Add evidence…" action (`POST /learnings/{id}/evidence` → link a work_task/decision/test_spec; supporting raises confidence, contradicting work_task lowers it) and a "Set confidence…" spinbox, so a UI-authored learning can clear the resolver's `confidence ≥ 1` gate and reach an agent's effective `active_learnings` (which attach by area+tier match, NOT by an explicit binding). **Cross-engagement promotion + curation (PI-337 / DEC-765, also implements REQ-367):** the Learnings panel also has a "Promote to system" detail button (shown only on an engagement-scoped row → `POST /learnings/{id}/promote-to-system`, flips a recurring engagement learning to a system default so every engagement inherits it — the DEC-373 multiplier), a "Cross-engagement candidates…" toolbar dialog (`GET /learnings/cross-engagement-candidates` — learnings recurring across ≥2 engagements, each promotable), and a "Curate area…" toolbar dialog (`POST /learnings/curate` — retire contradicted zero-confidence learnings). **Deferred:** agent search (`search_agents` repo fn has no REST endpoint — needs a backend endpoint + its own requirement). CRUD dialogs are thin `EntityCrudDialog` subclasses driven by `dialogs/_registry_schemas.py`; client methods added to `ui/client.py` (`list/get/create/patch/delete_*` for the 4 entities + `get_agent_profile_contract`/`_bindings` + `add/remove_agent_profile_*` binding helpers). Tests: `tests/crmbuilder_v2/ui/test_agent_registry_panels.py`. Governance recorded real-time (DEC-383): the requirement is human-approved (REQ-367 confirmed via DEC-749), PI-330 implements it under PRJ-066 in the dedicated release REL-026. Build-closure resolves PI-330 on `main` after merge.

**Reconcile Surface Redesign — REL-027 / PRJ-067 (PI-331…335, 06-28-26, MERGED PR #16 `96f0b003`, release SHIPPED).** Rebuilt the desktop three-way reconciliation *operator surface* on the existing REL-024 engine (compare design vs two live instances, capture/publish, transaction log, rollback, data-loss guard) — native Qt, replacing the old flat `QTreeWidget` panel. Design: `PRDs/product/crmbuilder-v2/reconcile-redesign-design.md`. No schema migration (the entity-settings columns already existed). What landed:
- **PI-331** (REQ-368/370) — `access/reconcile_compare.py` `three_way_compare` gained an entity-level **`existence`** rollup (one row/entity × design+A+B = present/absent/unknown) feeding the landing grid, and per-group **`object_groups`** partitioning rows into the six buckets fields/layouts/relations/formulas/settings/other (`object_type_for()`), feeding the collapsible detail tree. Flat `rows` retained.
- **PI-332** (REQ-369/371/375/376) — `access/reconcile_apply.py`: `capture_entity_setting` (entity-collection settings → design, the entity twin of field capture); `entity_for_member`/`publish_scope_for_member` (an object publishes via its **parent entity** — one generated program = one entity); `record_publish` (logs a publish txn + reconciles the instance membership snapshot). REST **`POST /reconcile/capture-setting`** + **`POST /reconcile/publish`** (the latter resolves the entity scope and drives the *existing safe scoped live publish* — backup+deploy+verify — then records the txn). Entity-settings attribute rows are now `actionable` (`RECONCILABLE_ENTITY_SETTINGS`). Client: `reconcile_capture_setting`/`reconcile_publish`.
- **PI-333** (REQ-370/373/378) — NEW `ui/panels/reconcile_models.py`: `ExistenceGridModel` (`QAbstractTableModel`) + `EntityDetailModel` (`QAbstractItemModel` two-level tree), colour+label state cells via Background/Foreground roles (never colour-only), operator-language labels. NEW `ui/panels/reconcile_grid.py` `ReconcileGridPanel`: pickers + Compare, existence grid with a sort/filter proxy (only-needs-attention), drill into the detail tree, grid lines + alternating colours + virtualization, empty-engagement guidance. **No web grid / browser engine.**
- **PI-334** (REQ-371/372/374) — pure **`plan_apply()`** router (the design is the **hub**: instance→design capture, design→instance publish, instance→instance via capture-then-publish; already-matching targets skipped). Detail-view apply bar (pick the correct-value source + target(s), Apply over every selected difference; unsupported reported); existence-grid whole-entity **promote** bar; every control relabelled with the chosen instance names.
- **PI-335** (REQ-377) — view-only config stays shown and selectable, **Apply is never disabled**, acting on a non-actionable item pops a plain configure-by-hand explanation and writes nothing; the txn log + guarded rollback folded into the panel as a **History tab** (REL-024 reversibility preserved); old `ui/panels/reconcile.py` flat panel + its test **deleted**, the **Reconcile** sidebar entry repointed to `ReconcileGridPanel`.

Tests: NEW `tests/crmbuilder_v2/ui/test_reconcile_grid.py` + extended `access/test_reconcile_compare.py`/`test_reconcile_apply.py`. Governance (real-time, DEC-383): requirements REQ-368…378 confirmed pre-build; built on branch `rel-027-reconcile-redesign` (Model A), governance against **SES-290** / build-closure conversation **CNV-233**, decisions **DEC-789/792/795/798/801**; PI-331…335 **Resolved** via `resolves` edges from CNV-233; the release record was walked to **`shipped`** in **manual** execution_mode (reconciliation + architecture review signoffs recorded, ship approval auto-recorded since all in-scope PIs resolved).

**Reconcile UI Quick Improvements — REL-037 / PRJ-074 (PI-350/351, 06-28-26, MERGED PR #17, release SHIPPED).** Three operator-requested follow-ons to the REL-027 reconcile surface, after Doug reviewed the shipped UI. No schema migration. What landed:
- **PI-350** (REQ-390/391) — the existence grid now labels the unknown-presence state **"Not audited"** (in `reconcile_models.STATE_LABELS`) instead of the ambiguous "n/a" that read as "not applicable" (the state means an entity was not in that location's last stored audit, since compare serves from stored audit, not live). Both reconciliation grids now **fill the full width** with no manual resize: a `_fill_width` helper stretches the critical text column (Entity / Difference) and sizes the value/location columns to content, replacing the old `resizeColumnsToContents` that left columns floating free.
- **PI-351** (REQ-392) — a **fast entity-only re-audit** from the reconcile grid: NEW `introspect/entity_audit.reconcile_entity_slice` re-reads ONE entity's full slice (presence + settings + fields + relationships + layouts) on a live instance and refreshes **only that entity's existing canonical members** to present/drifted/absent. It is a **refresh, not discovery** — it never creates new canonical records and never calls the global `mark_absent_missing` sweep, so it **cannot mark unrelated entities absent** (the failure mode REL-038 was fixing); it reuses the drift helpers from `reconcile.py`. NEW **`POST /instances/{id}/audit-entity/{entity_id}`** (inherits the `not_auditable`/`missing_credentials` gates via `_audit_introspection_client`); `client.audit_entity`. UI: a row-level **"Re-audit this entity"** button (both instance columns) + a per-cell right-click ("Re-audit on <instance>"), on a `QThread` worker with inline status + busy cursor, auto-refreshing the comparison on completion.

Tests: NEW `tests/crmbuilder_v2/introspect/test_entity_audit.py` (incl. the **does-not-touch-other-entities** safety property), extended `test_instance_audit_api.py` + `test_reconcile_grid.py`. Governance (real-time): requirements REQ-390/391/392 confirmed via **DEC-808** (provenance SES-292 / CNV-235 / topic TOP-111); built on branch `rel-037-reconcile-ui-quick` (Model A), build session **SES-295**, close-out conversation **CNV-242**, decisions **DEC-837/838**; PI-350/351 **Resolved**; release walked to **`shipped`** (manual mode). **Lane-wait note:** REL-037 sat at `ready` for hours behind REL-007 holding the single-occupancy dev lane; the reliable way to claim the lane the instant it frees (vs losing the race to automated schedulers) is a persistent monitor whose poll loop itself POSTs the `ready→development` transition when no release is in a lane state.

**Production Multi-Tenant API — PI-α Postgres foundation (06-02-26, on branch `pi-alpha-postgres`, pending merge + build-closure).** PI-123 (unified row-level `engagement_id` DB) is **Resolved**; its successor program under **PRJ-019** makes the API a true multi-tenant, multi-user Postgres service and retires the per-engagement-file + snapshot apparatus (design: `PRDs/product/crmbuilder-v2/production-multitenant-api-architecture.md`, decomposed into PI-α Postgres foundation → PI-γ RBAC, PI-β de-file/kill-snapshots in parallel). **PI-α is built** (design + build notes: `pi-alpha-postgres-foundation-architecture.md`; runbook: `pi-alpha-postgres-migration-runbook.md`): the v2 store now runs on **Postgres** when `CRMBUILDER_V2_DATABASE_URL` is set (e.g. `postgresql+psycopg://…`), falling back to the SQLite `db_path` when unset — **SQLite stays the default** until the prod migration is rehearsed. The PI-123 schema ports unchanged except for dialect handling: JSON columns are `JSON().with_variant(JSONB(), "postgresql")` aliases (`JSONColumn`/`JSONColumnNoneAsNull` in `models.py`); the SQLite-only CHECK idioms (`GLOB`, `json_*`, `IN (0,1)`, hex-GLOB) became dialect-rendering `ColumnElement` constructs (`_IdentifierFormatCheck`/`_NonEmptyJsonArrayCheck`/`_BooleanDomainCheck`/`_LowerHexCheck`) that emit **byte-identical SQLite** and `~`-regex/`jsonb_*`/boolean Postgres; `access/db.py` builds the engine dialect-conditionally (SQLite pragmas + `BEGIN IMMEDIATE` only on SQLite; PG gets `QueuePool` + `pre_ping`/`recycle`); `migrations/env.py` is dialect-aware. **Dual-head Alembic by design:** the SQLite batch chain (`migrations/` 0001-0039) is **not** replayed on Postgres — PG has its own tree at **`migrations/pg/`** (single `0001_pg_baseline` = `create_all`) and grows its own chain; never `alembic upgrade head` a Postgres DB through the SQLite chain. The one-shot data migration is `crmbuilder_v2/migration/sqlite_to_postgres.py` (`migrate(sqlite, pg_url)` — straight copy through Core, FK-ordered, self-FK two-pass, PG sequence reset, validated incl. the leak-test on PG). **Tests run on Postgres** via the `CRMBUILDER_V2_TEST_PG_URL`-gated `v2_env` (unset → per-test SQLite, unchanged); `.github/workflows/postgres-tests.yml` + `crmbuilder-v2/docker-compose.dev.yml` (PG16, port 55432) drive CI/dev. **Six columns were widened** (`…title`/`conversation_title`/`change_log.entity_identifier` → `Text`, `refs.relationship_kind` `String(32)→(64)`) because real data overran their `VARCHAR(n)` — SQLite never enforced the cap, Postgres does (Alembic `0039`). Remaining PI-α: PI-100 scale check, the Deployment phase (prod topology, managed PG, flip-the-default timing — deferred by design), and the governance build-closure (creates PI-α/β/γ under PRJ-019, resolves PI-α) on `main` after merge.

**De-file + kill-snapshots — PI-β (06-03-26, on branch `pi-beta-defile`, pending merge + build-closure; PRJ-019 / PI-126).** Finishes the runtime's exit from the per-engagement-file + JSON-snapshot world (design: `production-multitenant-api-architecture.md` D5–D8, refined in `pi-beta-defile-architecture.md`). **This supersedes the multi-engagement-file, `current_engagement.json` marker, meta-DB, activation-worker, and `db-export`/snapshot descriptions elsewhere in this file** where they conflict. There is now **one unified DB**; the active engagement is named **per request by the `X-Engagement` header** (identifier `ENG-NNN` or code), resolved by `api/scope_middleware` and applied as the row-level filter/stamp; `Settings.engagement_scoping_enabled` defaults **True**. What changed:
- **De-filed (slices 1–3):** the engagement registry is the unified DB's `engagements` table served by `/engagements` (the separate **meta DB** + its models/Alembic chain/`meta_session` are deleted); the **`current_engagement.json` marker is gone** (no marker fallback in the middleware); **`ui/activation_worker.py` is deleted** — engagement switching is a client-side context change (`StorageClient.set_active_engagement` → the header; `MainWindow.switch_engagement` refreshes panels), which removes the "Switching failed at step 3" subprocess-swap bug by construction. Also deleted: `dogfood_v0_5`, `lazy_migration`, `marker_guard`, the `/admin/active-engagement` reroute. `cli.run_api` no longer takes `--engagement` (the API serves the one DB; each request names its engagement). MCP/Claude-Code/desktop must send the header (full MCP per-engagement selection is a deferred follow-on; the prod MCP client currently sends none → unscoped, fine for the single-engagement dogfood since prod enforcement is off).
- **Snapshots killed (slice 4):** deleted `access/exporter.py` + the catalog exporter + `runtime/engagement_routing.py` (the export-dir gate); `session_scope()` is flush→commit only (no `export=`); removed `force_export`, `assert_export_dir_ready`, `Settings.export_dir`, the `EngagementExportDir*` exceptions, and the **git-tracked `db-export/` tree**. The desktop's file-watch `RefreshService` is deleted (it watched that tree); panels refresh on selection / lifecycle-ready / manual Refresh. `apply_close_out.py` stops regenerating snapshots (keeps the deposit-event log). The `engagement_export_dir` column + its CRUD field remain (vestigial; a schema-change pass will drop them).
- **Deferred:** PI-β slices 5–6 doc rewrites (this note + `v2-user-process-guide.md` are that work — `governance-recording-rules.md` was retired and its rules migrated into V2 under `TOP-013` by PI-130); MCP per-engagement selection; the vestigial `engagement_export_dir` column; repointing `scripts/enumerate_commits.py` off the absent db-export `commits.json`. Build-closure (records PI-β governance, resolves PI-126) lands on `main` after merge.
