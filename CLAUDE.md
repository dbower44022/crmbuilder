...
The process has nine phases executed in strict sequence:
...
Phase 3: Process Definition     → 1 conversation per business process
                                   done in dependency order within each domain
                                   produces one Word doc per process
Phase 4: Domain Reconciliation  → 1 conversation per domain
                                   synthesizes process docs into Domain PRD
Phase 5: Stakeholder Review     → outside Claude, via Google Docs
Phase 6: YAML Generation        → 1 conversation per domain
Phase 7: CRM Selection          → 1 conversation, produces CRM Evaluation
           and Deployment          Report + deployed instance
Phase 8: CRM Configuration      → tool-driven, YAML applied to CRM instance
Phase 9: Verification           → generated by CRM Builder tool
```
...
- Never mention specific product names (EspoCRM, WordPress, Moodle,
  ...
  are permitted in the CRM Evaluation Report (Phase 7).
