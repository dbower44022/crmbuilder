...
- **Coverage baseline cutoff (durable):** `/coverage/capabilities?since=<iso>` (or the `CRMBUILDER_V2_PROVENANCE_BASELINE` default) splits legacy pre-model debt (`baseline_summary`) from live gaps, so the report stops drowning in the pre-provenance corpus. The default is set durably in the gitignored, repo-rooted **`crmbuilder-v2/data/crmbuilder.env`** — read by `Settings` on every process start (standalone or UI-spawned), real env vars still override, tests forced hermetic via the `v2_env` fixture. This `.env` is now the home for any durable `CRMBUILDER_V2_*` default.
