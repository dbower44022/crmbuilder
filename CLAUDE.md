...
`PRDs/application/CRM-Builder-Document-Production-Process.docx`

### Process Summary

The process has six phases executed in strict sequence:

```
Phase 1: Master PRD             → 1 conversation, produces Word doc
Phase 2: Process Definition     → 1 conversation per business process
                                   done in dependency order within each domain
                                   produces one Word doc per process
Phase 3: Domain Reconciliation  → 1 conversation per domain
                                   synthesizes process docs into Domain PRD
Phase 4: Stakeholder Review     → outside Claude, via Google Docs
Phase 5: YAML Generation        → 1 conversation per domain
Phase 6: Verification           → generated by CRM Builder tool
```
...
- When scope changes are discovered mid-conversation, stop and fix the
  Master PRD before continuing (see process doc Section 9)

### PRD Content Rules

- Never mention specific product names (EspoCRM, WordPress, Moodle,
  Constant Contact, etc.) in Master PRDs, process documents, or
  Domain PRDs. These are implementation details only.
- Every requirement and data item must have a unique identifier
  following the scheme in the process document Section 5.
- Process documents are not complete until all six required sections
  contain field-level detail (see process doc Section 3.2).
