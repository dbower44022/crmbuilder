...
**Automatic at session start (PI-437 / REQ-540):** the `SessionStart` hook in `.claude/settings.json` runs `crmbuilder-v2/src/crmbuilder_v2/session_context.py`, which reads the effective governance rules whose audience includes a Claude Code session plus the active preferences (steps 2 and 3 below) and places them in context before the first prompt. If the store is unreachable it prints the last snapshot (`crmbuilder-v2/data/session-context.snapshot.md`, gitignored) under a visible banner. The manual reads below remain the fallback and cover steps 1, 4 and 5.

**Enforced rules are checked before a command runs (PI-439 / REQ-542):** the `PreToolUse` hook runs `crmbuilder-v2/src/crmbuilder_v2/rule_check.py` on every Bash command and evaluates each enforced rule's check (`forbidden_command`, `required_trailer`, `protected_path`) against it. An `enforced` rule that fails blocks the command with the rule named; an `enforced_with_override` rule can be waved through by prefixing the command with `GVR_OVERRIDE='GVR-NNN: <reason>'`, and the reason is recorded in the store (`/governance-rules/{id}/enforcement-overrides`). Rules are cached for ten minutes in the gitignored `crmbuilder-v2/data/rule-checks.snapshot.json`; with no store and no snapshot the hook fails open with a warning.
