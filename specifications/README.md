...
| `governance-recording/process-prd-definition-process.md` | Process PRD for the Process PRD Definition Process, the meta process describing how a Process PRD is authored: twelve steps from anchoring on a confirmed requirement through drafting with open questions, one ruling at a time, a single approval gate, write and verify, render and commit, and resolve after the push, each citing the PI-087 run's actual records. A dated render of process record PROC-011 in the V2 store, which is the source (DEC-1021). | 0.3 — DISCUSSION DRAFT, render of a supporting process record |
...
| 0.4 | 09-04-26 01:20 | Added the meta-process render, `governance-recording/process-prd-definition-process.md` v0.3 (SES-388 / CNV-360, PI-088). |
