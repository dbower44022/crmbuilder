...
# Fallback file_path for `other`-kind work_tickets (sessions opened ad hoc
# without a committed kickoff file). The API requires a non-empty repo-
# relative path on every work_ticket; we point the field at the kickoff doc
# for this backfill, which is the closest authoritative artifact describing
# the missing-kickoff cases.
_OTHER_KIND_FALLBACK_PATH = (
    "PRDs/product/crmbuilder-v2/pi-025-prior-conversations-backfill-kickoff.md"
)
...
def create_work_tickets() -> bool:
    ...
    for i, item in enumerate(_ITEMS):
        ...
        body: dict[str, Any] = {
            ...
            "work_ticket_file_path": item["wt_path"] or _OTHER_KIND_FALLBACK_PATH,
        }
...
def create_conversations() -> bool:
    ...
    for i, item in enumerate(_ITEMS):
        ...
        ses_date = _normalize_ses_date(_SES_DATE.get(item["ses"], "2026-05-23"))
