...
def seed_planning_item(base: str, eng: str, start_design: bool) -> dict:
    ...
    prj = api("POST", base, "/projects", engagement=eng, body={
    ...
    pi = api("POST", base, "/planning-items", engagement=eng, body={
        "title": "Add an end-to-end proof file",
        "item_type": "pending_work",
        "status": "Draft",
        "description": (
            "Create a single small marker file to prove the ADO loop runs end to end. "
            "The storage Work Task should create a file `ado-e2e-proof.txt` in the repo "
            "root containing one line, then commit it. No other changes."
        ),
        "executive_summary": (
            "A deliberately trivial storage Planning Item used to exercise the ADO "
            "orchestration driver end to end with real agents. Its only deliverable is a "
            "single marker file (`ado-e2e-proof.txt`) created and committed in the throwaway "
            "sandbox repo, so the run is fast, safe, and easy to verify by eye. It exists to "
            "validate the scope -> reconcile -> build -> review loop and to tune the real-agent "
            "prompts, not to deliver product value; it lives in the isolated ADOTEST engagement "
            "so it never touches the CRMBUILDER backlog."
        ),
    })["identifier"]
    ...
    scoped = api("POST", base, f"/workstreams/{design}/scope", engagement=eng, body={
        "work_tasks": [{
            "title": "Create the end-to-end proof file",
            "area": AREA,
            "description": (
                "In the repository root, create a file named `ado-e2e-proof.txt` whose "
                "entire contents are the single line: `ADO end-to-end proof`. "
                "That is the whole task. Commit it. Do not touch anything else."
            ),
        }],
    })
    ...
    return {"project": prj, "pi": pi, "phases": phases, "work_task": wtk}
...
def main() -> int:
    ...
    ensure_repo(repo)
    s = seed_planning_item(base, eng, args.start_design)
    ...
    if args.start_design:
        ...
        print(f"  CRMBUILDER_V2... ; uv run crmbuilder-v2-runtime --work-task {s['work_task']} \\")
        print(f"      --repo-root {repo} --base-branch main --tier developer\n")
    else:
        print("Rung 4 — full driver: one PI scoped→reconciled→built→In Review:")
        print(f"  uv run crmbuilder-v2-ado {s['pi']} --repo-root {repo} --base-branch main\n")
        print("  (re-run this seed with --start-design for the Rung-2 single-agent test instead)\n")
    print("Watch / verify:")
