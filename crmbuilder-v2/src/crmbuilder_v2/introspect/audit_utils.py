"""Reverse-mapping utilities for CRM introspection.

Ported from V1 ``espo_impl.core.audit_utils`` for PI-187.
...
The V1 module imported ``NATIVE_ENTITIES`` from the Qt UI module
``espo_impl.ui.confirm_delete_dialog``. To keep this package free of any
Qt/UI coupling, a copy of that constant is defined here instead.
"""
...
#:
#: Copied verbatim from V1 ``espo_impl.ui.confirm_delete_dialog`` so this
#: package never imports from the Qt UI layer (PI-187 scope rule).
...
def strip_field_c_prefix(api_name: str, entity_is_native: bool = True) -> str:
    ...
    custom fields keep their natural names, so a name that legitimately
    begins with a lowercase ``c`` followed by an uppercase letter — e.g.
    ``cBMValueProvided`` from a label "CBM Value Provided" — must be left
    untouched; stripping it corrupts the field identity on re-deploy
    (REQ-342).

    ``cContactType`` (native) -> ``contactType``;
    ``cBMValueProvided`` (custom entity) -> ``cBMValueProvided``.
    ...
    :param entity_is_native: Whether the field's parent entity is native.
        ...
        ``True`` for callers without entity context.
...
def strip_entity_c_prefix(api_name: str) -> str:
    ...
    Native entity names are returned unchanged. Names that don't follow
    the ``C{Uppercase}...`` pattern are returned unchanged (including
    names where the second character isn't uppercase, which would never
    be a valid custom entity name).
...
def classify_entity(
    ...
    :returns: :class:`EntityClass` classification.
    """
...
def classify_field(
    ...
    :param entity_type: Entity type string ("Person", "Company", "Base",
        "Event") for native field detection.
    :returns: :class:`FieldClass` classification.
    """
