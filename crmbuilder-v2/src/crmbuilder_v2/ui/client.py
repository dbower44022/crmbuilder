...
from crmbuilder_v2.segments import client_mixins
...
# Segment packages contribute their record methods as mixins (PI-508 /
# REQ-591); the registry lists them, so a new record type inside a package
# needs no edit here. Method names are unchanged for every caller.
class StorageClient(*client_mixins(), _StorageClientBase):
    """Synchronous HTTP client for the v2 storage REST API.

    The transport, envelope handling and error mapping live in
    ``_StorageClientBase``; each segment package adds its record methods.
    """
