"""Charter panel — versioned view + replace flow per slice E (v0.2).

Slice E (v0.1) wired the read-only versioned list and detail. Slice E
(v0.2) adds:

* "New Version" button in the toolbar (opens
  :class:`CharterReplaceDialog`).
...
from crmbuilder_v2.ui.dialogs.charter_replace import CharterReplaceDialog
...
_log = logging.getLogger("crmbuilder_v2.ui.panels.charter")

_REFERENCES_ENTITY_TYPE = "charter"
_REFERENCES_ENTITY_ID = "charter"


class CharterPanel(VersionedPanel):
    """Charter — singleton with full version history and replace flow."""
    ...
    # ------------------------------------------------------------------
    # Detail pane (override base _render_payload composition)
    # ------------------------------------------------------------------
    ...
    def _on_new_version_clicked(self) -> None:
        ...
        if current is None:
            ErrorDialog(
                "No current charter",
                "There is no current charter version. Cannot create a new version.",
                parent=self,
            ).exec()
            return
        dialog = CharterReplaceDialog(
    ...
    def _on_make_current(self, version: int) -> None:
        ...
        confirm.setText(
            f"Make charter version {version} the current version?"
        )
        ...
        worker = run_in_thread(
            lambda v=version: self._client.make_charter_version_current(v),
    ...
    def _on_make_current_success(self, _result: object) -> None:
        # Allow re-selection of the (now-current) row on next refresh.
