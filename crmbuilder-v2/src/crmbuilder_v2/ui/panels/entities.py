"""Entities panel — the second methodology entity type (UI v0.4 slice C).

A ``ListDetailPanel`` for the ``entity`` entity, registered as the
second entry under the Methodology sidebar group (after Domains).
Mirrors slice B's ``DomainsPanel`` with entity-specific adjustments: a
master table with a Show-deleted toggle and a New button, a structured
read-only detail pane with Edit/Delete (or Restore) actions, and a
right-click context menu.

Detail pane layout follows ``entity.md`` section 3.6.3: identifier
(read-only label), name, description (read-only editors),
``entity_notes`` under a collapsible "Internal notes" header collapsed
by default, status, and the shared ``ReferencesSection`` widget. Unlike
the Domains panel — whose ``ReferencesSection`` is inbound-only in
v0.4 — this is the first methodology panel that surfaces *outgoing*
edges: the ``entity_scopes_to_domain`` affiliations the user attaches
from the detail pane's "Add reference" affordance. No master-pane
Domains column ships in v0.4 (``entity.md`` section 3.6.2, PI-009).
"""
...
from crmbuilder_v2.ui.dialogs._entity_schema import status_choices
...
class EntitiesPanel(ListDetailPanel):
    """Entities panel with read + write surfaces (UI v0.4 slice C)."""
    ...
    def __init__(self, client, parent=None):
        ...
        self._new_button = primary_button("New Entity")
        self._new_button.setObjectName("new_entity_button")
        self._new_button.clicked.connect(self._on_new_entity_clicked)
    ...
    def _strikethrough_for_record(self, record: dict[str, Any]) -> bool:
        return record.get("entity_deleted_at") is not None
    ...
    def render_detail(
        ...
        identifier = record.get("entity_identifier") or ""
        is_deleted = record.get("entity_deleted_at") is not None
        ...
        if not is_deleted:
            ...
            delete_btn.setObjectName("delete_entity_button")
        ...
        outer.addWidget(_heading_label(record.get("entity_name") or "(unnamed)"))
        ...
        identifier_label.setObjectName("entity_identifier_value")
        identifier_label.setTextInteractionFlags(
            Qt.TextInteractionFlag.TextSelectableByMouse
        )
        form.addRow("Identifier", identifier_label)

        name_value = _read_only_line(record.get("entity_name") or "")
        name_value.setObjectName("entity_name_value")
        form.addRow(required_label("Name"), name_value)

        description_value = _read_only_text(
            record.get("entity_description") or "",
            placeholder=_DESCRIPTION_PLACEHOLDER,
        )
        description_value.setObjectName("entity_description_value")
        form.addRow(required_label("Description"), description_value)
        outer.addLayout(form)

        # Field 4: entity_notes under a collapsible "Internal notes"
        ...
        notes_value = _read_only_text(record.get("entity_notes") or "")
        notes_value.setObjectName("entity_notes_value")
        notes_section = CollapsibleSection(
            "Internal notes", notes_value, expanded=False
        )
        notes_section.setObjectName("entity_notes_toggle")
        outer.addWidget(notes_section)

        # Field 5: entity_status — combo box restricted to the valid
        ...
        current_status = record.get("entity_status") or "candidate"
        status_combo = QComboBox()
        status_combo.setObjectName("entity_status_value")
        ...
        # widget is always present for v0.5+ inbound kinds too.
    ...
    # ------------------------------------------------------------------
    # Identifier addressing (entity uses ``entity_identifier``, not the
    # base's ``identifier`` key)
    # ------------------------------------------------------------------

    def _select_by_identifier(self, identifier: str) -> bool:
        for row, record in enumerate(self._records):
            if record.get("entity_identifier") == identifier:
    ...
    def _currently_selected_identifier(self) -> str | None:
        ...
        if 0 <= row < len(self._records):
            ident = self._records[row].get("entity_identifier")
    ...
    def _build_context_menu(self, index: QModelIndex) -> QMenu:
        menu = QMenu(self)
        if not index.isValid():
            new_action = menu.addAction("New entity")
            new_action.triggered.connect(self._on_new_entity_clicked)
            return menu
        ...
        if record.get("entity_deleted_at") is not None:
    ...
    def _on_new_entity_clicked(self) -> None:
        dialog = EntityCreateDialog(self._client, self)
    ...
    def _on_edit_clicked(self, record: dict[str, Any]) -> None:
        ...
        if not identifier:
            return
        try:
            fresh = self._client.get_entity(identifier)
            ...
            _log.warning("Entity error loading %s for edit: %s", identifier, exc)
            ErrorDialog(
                title="Could not load entity",
                message="Could not load the latest version of this entity.",
                detail=str(exc),
                parent=self,
            ).exec()
            return

        dialog = EntityEditDialog(self._client, fresh, self)
        if dialog.exec() == QDialog.DialogCode.Accepted:
            self.refresh()
    ...
    def _on_delete_clicked(self, record: dict[str, Any]) -> None:
        ...
        if not identifier:
            return
        dialog = EntityDeleteDialog(self._client, identifier, name, self)
        if dialog.exec() == QDialog.DialogCode.Accepted:
            self.refresh()
    ...
    def _on_restore_clicked(self, record: dict[str, Any]) -> None:
        ...
        confirm.setText(
            ...
            "It will reappear in the default Entities list."
        )
        ...
        if confirm.exec() != QMessageBox.StandardButton.Yes:
            return
        try:
            self._client.restore_entity(identifier)
        except NotFoundError:
            self.refresh()
            return
        except StorageConnectionError as exc:
            ...
            _log.warning("Entity error restoring %s: %s", identifier, exc)
            ErrorDialog(
                title="Could not restore entity",
                message=(
                    "An error occurred while restoring the entity. "
                    "Please try again."
                ),
                detail=str(exc),
                parent=self,
