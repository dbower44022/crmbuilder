"""Projects panel — the first governance entity type (UI v0.7).
...
_LIFECYCLE_TIMESTAMPS = [
    ...
    ("Completed", "project_completed_at"),
...
class ProjectsPanel(ListDetailPanel):
    """Projects panel with read + write surfaces (UI v0.7)."""
    ...
    def __init__(self, client, parent=None):
        ...
        self._new_button = primary_button("New Project")
    ...
    def fetch_records(self) -> list[dict[str, Any]]:
        records = self._client.list_projects(include_deleted=self._include_deleted)
        # PI-108: formatted Created/Updated synthetic columns.
        for r in records:
            r["created_at_display"] = format_timestamp(r.get("project_created_at"))
            r["updated_at_display"] = format_timestamp(r.get("project_updated_at"))
        return records

    def list_columns(self) -> list[ColumnSpec]:
        return [
            ColumnSpec(field="project_identifier", title="Identifier", width=120),
            ColumnSpec(field="project_name", title="Name"),
            ColumnSpec(field="project_status", title="Status", width=110),
    ...
    def _strikethrough_for_record(self, record: dict[str, Any]) -> bool:
        return record.get("project_deleted_at") is not None
    ...
    def fetch_detail_extras(self, record: dict[str, Any]) -> dict[str, Any]:
        identifier = record.get("project_identifier")
        if not identifier:
    ...
    def render_detail(self, record: dict[str, Any], extras: dict[str, Any]) -> QWidget:
        ...
        is_deleted = record.get("project_deleted_at") is not None
        ...
        outer.addWidget(heading_label(record.get("project_name") or "(unnamed)"))
        ...
        notes_section = CollapsibleSection(
            "Internal notes",
            read_only_text(record.get("project_notes") or ""),
            expanded=False,
        )
        ...
        current_status = record.get("project_status") or "planned"
        ...
        outer.addWidget(
            created_updated_section(
                record, "project_created_at", "project_updated_at"
            )
        )
    ...
    def _select_by_identifier(self, identifier: str) -> bool:
        for row, record in enumerate(self._records):
            if record.get("project_identifier") == identifier:
    ...
    def _currently_selected_identifier(self) -> str | None:
        ...
        if not idx.isValid() or not (0 <= idx.row() < len(self._records)):
            return None
        ident = self._records[idx.row()].get("project_identifier")
    ...
    def _build_context_menu(self, index: QModelIndex) -> QMenu:
        ...
        if record.get("project_deleted_at") is not None:
    ...
    def _on_new_clicked(self) -> None:
        dialog = ProjectCreateDialog(self._client, self)
    ...
    def _on_edit_clicked(self, record: dict[str, Any]) -> None:
        ...
        if not identifier:
            return
        try:
            fresh = self._client.get_project(identifier)
            ...
            return
        dialog = ProjectEditDialog(self._client, fresh, self)
        if dialog.exec() == QDialog.DialogCode.Accepted:
            self.refresh()
    ...
    def _on_delete_clicked(self, record: dict[str, Any]) -> None:
        ...
        if not identifier:
            return
        dialog = ProjectDeleteDialog(self._client, identifier, name, self)
        if dialog.exec() == QDialog.DialogCode.Accepted:
            self.refresh()
    ...
    def _on_restore_clicked(self, record: dict[str, Any]) -> None:
        ...
        if confirm.exec() != QMessageBox.StandardButton.Yes:
            return
        try:
            self._client.restore_project(identifier)
        except NotFoundError:
            self.refresh()
            return
        except StorageConnectionError as exc:
