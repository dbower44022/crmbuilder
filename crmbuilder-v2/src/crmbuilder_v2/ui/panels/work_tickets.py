"""Work tickets panel — the fourth governance entity type (UI v0.7)."""
...
def _status_choices(current: str | None) -> list[str]:
    ...
    return sorted({current} | set(WORK_TICKET_STATUS_TRANSITIONS.get(current, frozenset())))


class WorkTicketsPanel(ListDetailPanel):
    def __init__(self, client, parent=None):
        ...
        self._new_button = primary_button("New Work Ticket")
    ...
    def fetch_records(self) -> list[dict[str, Any]]:
        records = self._client.list_work_tickets(
            include_deleted=self._include_deleted
        )
        # PI-108: formatted Created/Updated synthetic columns.
        for r in records:
            r["created_at_display"] = format_timestamp(
                r.get("work_ticket_created_at")
            )
            r["updated_at_display"] = format_timestamp(
                r.get("work_ticket_updated_at")
            )
        return records

    def list_columns(self) -> list[ColumnSpec]:
        return [
            ColumnSpec(field="work_ticket_identifier", title="Identifier", width=110),
            ColumnSpec(field="work_ticket_title", title="Title"),
            ColumnSpec(field="work_ticket_kind", title="Kind", width=160),
            ColumnSpec(field="work_ticket_status", title="Status", width=110),
    ...
    def _strikethrough_for_record(self, record: dict[str, Any]) -> bool:
        return record.get("work_ticket_deleted_at") is not None
    ...
    def fetch_detail_extras(self, record: dict[str, Any]) -> dict[str, Any]:
        identifier = record.get("work_ticket_identifier")
        if not identifier:
            return {"references": {"as_source": [], "as_target": []}}
        return {"references": self._client.list_references_touching("work_ticket", identifier)}
    ...
    def render_detail(self, record: dict[str, Any], extras: dict[str, Any]) -> QWidget:
        ...
        is_deleted = record.get("work_ticket_deleted_at") is not None
        ...
        outer.addWidget(heading_label(record.get("work_ticket_title") or "(untitled)"))
        ...
        form.addRow(required_label("Title"), read_only_line(record.get("work_ticket_title") or ""))
        form.addRow(required_label("Description"), read_only_text(record.get("work_ticket_description") or ""))
        form.addRow(required_label("Kind"), read_only_line(record.get("work_ticket_kind") or ""))
        form.addRow(required_label("File path"), read_only_line(record.get("work_ticket_file_path") or ""))
        outer.addLayout(form)

        outer.addWidget(CollapsibleSection(
            "Internal notes",
            read_only_text(record.get("work_ticket_notes") or ""),
            expanded=False,
        ))

        status_row = QFormLayout()
        status_row.setRowWrapPolicy(QFormLayout.RowWrapPolicy.WrapAllRows)
        current_status = record.get("work_ticket_status") or "drafted"
        ...
        outer.addWidget(
            created_updated_section(
                record, "work_ticket_created_at", "work_ticket_updated_at"
            )
        )
        ...
        refs = ReferencesSection(
            "work_ticket", identifier, extras.get("references") or {}, client=self._client
        )
    ...
    def _select_by_identifier(self, identifier: str) -> bool:
        for row, record in enumerate(self._records):
            if record.get("work_ticket_identifier") == identifier:
    ...
    def _build_context_menu(self, index: QModelIndex) -> QMenu:
        ...
        if record.get("work_ticket_deleted_at") is not None:
    ...
    def _on_new_clicked(self) -> None:
        dialog = WorkTicketCreateDialog(self._client, self)
    ...
    def _on_edit_clicked(self, record: dict[str, Any]) -> None:
        ...
        if not identifier:
            return
        try:
            fresh = self._client.get_work_ticket(identifier)
            ...
            ErrorDialog(title="Could not load work ticket", message=str(exc), parent=self).exec()
            return
        dialog = WorkTicketEditDialog(self._client, fresh, self)
        if dialog.exec() == QDialog.DialogCode.Accepted:
            self.refresh()

    def _on_delete_clicked(self, record: dict[str, Any]) -> None:
        identifier = record.get("work_ticket_identifier") or ""
        title = record.get("work_ticket_title") or ""
        if not identifier:
            return
        dialog = WorkTicketDeleteDialog(self._client, identifier, title, self)
        if dialog.exec() == QDialog.DialogCode.Accepted:
            self.refresh()
    ...
    def _on_restore_clicked(self, record: dict[str, Any]) -> None:
        ...
        if confirm.exec() != QMessageBox.StandardButton.Yes:
            return
        try:
            self._client.restore_work_ticket(identifier)
        except NotFoundError:
            self.refresh()
            return
        except StorageConnectionError as exc:
