"""Conversations panel — the second governance entity type (UI v0.7)."""
...
def _status_choices(current: str | None) -> list[str]:
    ...
    return sorted({current} | set(CONVERSATION_STATUS_TRANSITIONS.get(current, frozenset())))


class ConversationsPanel(ListDetailPanel):
    def __init__(self, client, parent=None):
        ...
        self._new_button = primary_button("New Conversation")
    ...
    def fetch_records(self) -> list[dict[str, Any]]:
        records = self._client.list_conversations(
            include_deleted=self._include_deleted
        )
        # PI-108: formatted Created/Updated synthetic columns.
        for r in records:
            r["created_at_display"] = format_timestamp(
                r.get("conversation_created_at")
            )
            r["updated_at_display"] = format_timestamp(
                r.get("conversation_updated_at")
            )
        return records

    def list_columns(self) -> list[ColumnSpec]:
        return [
            ColumnSpec(field="conversation_identifier", title="Identifier", width=120),
            ColumnSpec(field="conversation_title", title="Title"),
            ColumnSpec(field="conversation_status", title="Status", width=130),
    ...
    def _strikethrough_for_record(self, record: dict[str, Any]) -> bool:
        return record.get("conversation_deleted_at") is not None
    ...
    def fetch_detail_extras(self, record: dict[str, Any]) -> dict[str, Any]:
        identifier = record.get("conversation_identifier")
        if not identifier:
            return {"references": {"as_source": [], "as_target": []}}
        return {
            "references": self._client.list_references_touching("conversation", identifier),
        }
    ...
    def render_detail(self, record: dict[str, Any], extras: dict[str, Any]) -> QWidget:
        ...
        identifier = record.get("conversation_identifier") or ""
        is_deleted = record.get("conversation_deleted_at") is not None
        ...
        outer.addWidget(heading_label(record.get("conversation_title") or "(untitled)"))
        ...
        form.addRow(required_label("Title"), read_only_line(record.get("conversation_title") or ""))
        form.addRow(required_label("Purpose"), read_only_line(
            record.get("conversation_purpose") or "", placeholder="One sentence"))
        ...
        outer.addWidget(CollapsibleSection(
            "Internal notes",
            read_only_text(record.get("conversation_notes") or ""),
            expanded=False,
        ))

        status_row = QFormLayout()
        status_row.setRowWrapPolicy(QFormLayout.RowWrapPolicy.WrapAllRows)
        current_status = record.get("conversation_status") or "planned"
        ...
        outer.addWidget(
            created_updated_section(
                record, "conversation_created_at", "conversation_updated_at"
            )
        )
        ...
        refs = ReferencesSection(
            "conversation", identifier, extras.get("references") or {}, client=self._client
        )
    ...
    def _select_by_identifier(self, identifier: str) -> bool:
        for row, record in enumerate(self._records):
            if record.get("conversation_identifier") == identifier:
    ...
    def _build_context_menu(self, index: QModelIndex) -> QMenu:
        ...
        if record.get("conversation_deleted_at") is not None:
    ...
    def _on_new_clicked(self) -> None:
        dialog = ConversationCreateDialog(self._client, self)
    ...
    def _on_edit_clicked(self, record: dict[str, Any]) -> None:
        ...
        if not identifier:
            return
        try:
            fresh = self._client.get_conversation(identifier)
            ...
            ErrorDialog(title="Could not load conversation", message=str(exc), parent=self).exec()
            return
        dialog = ConversationEditDialog(self._client, fresh, self)
        if dialog.exec() == QDialog.DialogCode.Accepted:
            self.refresh()

    def _on_delete_clicked(self, record: dict[str, Any]) -> None:
        identifier = record.get("conversation_identifier") or ""
        title = record.get("conversation_title") or ""
        if not identifier:
            return
        dialog = ConversationDeleteDialog(self._client, identifier, title, self)
        if dialog.exec() == QDialog.DialogCode.Accepted:
            self.refresh()
    ...
    def _on_restore_clicked(self, record: dict[str, Any]) -> None:
        ...
        confirm.setWindowTitle("Restore conversation")
        ...
        if confirm.exec() != QMessageBox.StandardButton.Yes:
            return
        try:
            self._client.restore_conversation(identifier)
        except NotFoundError:
            self.refresh()
            return
        except StorageConnectionError as exc:
