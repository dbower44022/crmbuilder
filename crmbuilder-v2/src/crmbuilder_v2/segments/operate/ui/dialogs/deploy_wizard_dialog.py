...
1. **Providers** — how the CRM's address is managed (Cloudflare, or manual DNS
   at the domain's own provider — PI-566 / REQ-642), and are the credentials
   that choice needs in place? (opens :class:`ProviderCredentialsDialog`)
2. **Server** — instance name; region, size, image and account SSH keys from
   the live DigitalOcean catalog.
3. **Domain** — Cloudflare zone and subdomain (with the resulting name shown),
   or for manual DNS the full address; Let's Encrypt email; explains why the
...
class DeployWizardDialog(QDialog):
    ...
    def _build_providers_page(self) -> QWidget:
        ...
        v = QVBoxLayout(page)
        intro = QLabel(
            "A deploy run creates a server in DigitalOcean and the DNS record for "
            "the CRM's address. With Cloudflare the run creates the record itself; "
            "with manual DNS you add it by hand at the domain's own DNS provider "
            "when the run shows it, and no Cloudflare credential is needed."
        )
        intro.setWordWrap(True)
        v.addWidget(intro)
        self.dns_mode = QComboBox()
        self.dns_mode.setObjectName("wizard_dns_mode")
        self.dns_mode.addItem("Cloudflare — the run creates the record", DNS_MODE_CLOUDFLARE)
        self.dns_mode.addItem(
            "Manual DNS — I add the record at the domain's own DNS provider", DNS_MODE_MANUAL
        )
        self.dns_mode.currentIndexChanged.connect(self._dns_mode_changed)
        mode_form = QFormLayout()
        mode_form.addRow(required_label("DNS"), self.dns_mode)
    ...
    def _build_domain_page(self) -> QWidget:
        page = QWidget()
        form = QFormLayout(page)
        self._domain_form = form
        self.full_address = QLineEdit()
        self.full_address.setObjectName("wizard_full_address")
        self.full_address.setPlaceholderText("crm.example.org")
        form.addRow(required_label("CRM address"), self.full_address)
        self.zone = QComboBox()
        ...
        self._domain_hint = QLabel("")
        self._domain_hint.setObjectName("wizard_domain_hint")
        self._domain_hint.setWordWrap(True)
        self._domain_hint.setStyleSheet("color: #666;")
        form.addRow("", self._domain_hint)
        self._apply_dns_mode()
        return page
    ...
    def manual_dns(self) -> bool:
        """Whether the operator chose manual DNS on the providers page."""
        return self.dns_mode.currentData() == DNS_MODE_MANUAL

    def _needed_providers(self) -> tuple[tuple[str, str], ...]:
        if self.manual_dns:
            return (("digitalocean", "DigitalOcean"),)
        return (("digitalocean", "DigitalOcean"), ("cloudflare", "Cloudflare"))

    def _dns_mode_changed(self) -> None:
        self._apply_dns_mode()
        if not self.manual_dns and not self._zones:
            self._load_zones()

    def _apply_dns_mode(self) -> None:
        """Show the domain page's rows for the chosen DNS mode."""
        if not hasattr(self, "_domain_form"):
            return
        manual = self.manual_dns
        self._domain_form.setRowVisible(self.full_address, manual)
        for widget in (self.zone, self.subdomain, self.fqdn):
            self._domain_form.setRowVisible(widget, not manual)
        if manual:
            self._domain_hint.setText(
                "Manual DNS: once the server has its IP address the run shows the "
                "A record to add at the domain's DNS provider, then waits for it to "
                "go live. Leave any proxy or forwarding off, so the certificate "
                "challenge and SSH reach the server directly."
            )
        else:
            self._domain_hint.setText(
    ...
    def validate_page(self, index: int) -> str | None:
        ...
        if index == PAGE_PROVIDERS:
            missing = [
                name
                for key, name in self._needed_providers()
            ...
            if self.manual_dns:
                if "." not in self.full_address.text().strip():
                    return "Enter the CRM's full address, for example crm.example.org."
            else:
    ...
    def build_body(self) -> dict[str, Any]:
        ...
        body: dict[str, Any] = {
            ...
            "dns_mode": self.dns_mode.currentData(),
        ...
        if self.manual_dns:
            ...
            body["zone_id"] = self.zone.currentData()
            body["zone_name"] = self.zone.currentText()
            body["subdomain"] = self.subdomain.text().strip().lower()
    ...
    def render_review(self) -> str:
        ...
        return (
            ...
            f"Address:            https://{self.address()}\n"
            f"DNS:                {self._dns_review()}\n"
    ...
    def _dns_review(self) -> str:
        if self.manual_dns:
            return "manual DNS — you add the A record when the run shows it"
        return "Cloudflare — the run creates the record"
    ...
    def _load_catalogs(self) -> None:
        """Load the catalogs the configured credentials and DNS mode allow."""
