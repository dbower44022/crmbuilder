...
Rebuilds ``ck_ref_relationship`` (last rebuilt by 0109) and
``ck_session_medium`` (set by 0021) from the live vocabulary — both supersets,
no row invalidated; inspector-guarded so the chain is safe to enter mid-stream.
Mirrors 0135 / 0137.
...
SQLite chain head 0137 -> 0138. Companion PG-chain delta:
``migrations/pg/versions/0095_pi_462_withdraws_and_claude_code.py``.
...
revision: str = "0138_pi_462_withdraws_and_claude_code"
down_revision: str | None = "0137_pi_418_portal_layout_types"
...
def _rebuild_session_medium_check(mediums: frozenset[str]) -> None:
    ...
    with op.batch_alter_table("sessions") as batch:
        batch.drop_constraint("ck_session_medium", type_="check")
        batch.create_check_constraint(
            "ck_session_medium", _check_in("session_medium", mediums)
        )
