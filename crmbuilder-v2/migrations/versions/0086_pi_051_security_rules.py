"""PI-051 (REQ-128 / REQ-129) — security-rule entity tables.

Creates ``field_permission_rules`` (``FPR-NNN``) and ``field_visibility_rules``
(``FVR-NNN``) — the two security design records (DEC-698). Both are created from
the ORM ``__table__`` with ``checkfirst`` (idempotent on the
create_all-then-upgrade-head test path).

Rebuilds ``ck_changelog_entity_type`` and the ``refs`` ``ck_ref_source_type`` /
``ck_ref_target_type`` CHECKs to admit the two new entity types — the known
gotcha: tests build via create_all and miss it, the live DB 500s without it
(see 0034 / 0043 / 0045 / 0048 / 0052 / 0054 / 0060 / 0081 / 0082).
...
the chain is safe to enter mid-stream. SQLite chain head 0085 -> 0086; companion
PG delta ``migrations/pg/versions/0043_pi_051_security_rules.py``.
"""
...
revision: str = "0086_pi_051_security_rules"
down_revision: str | None = "0085_pi_301_agent_capability_description"
branch_labels: str | Sequence[str] | None = None
depends_on: str | Sequence[str] | None = None

_NEW_TYPES = frozenset({"field_permission_rule", "field_visibility_rule"})
