...
SQLite chain head 0138 -> 0139. Companion PG-chain delta:
``migrations/pg/versions/0096_pi_488_session_opening_fields.py``.
...
revision: str = "0139_pi_488_session_opening_fields"
down_revision: str | None = "0138_pi_462_withdraws_and_claude_code"
...
def upgrade() -> None:
    ...
    with op.batch_alter_table(_TABLE) as batch:
        if "session_opening_answer" not in present:
            batch.add_column(sa.Column("session_opening_answer", sa.Text(), nullable=True))
        if "session_kind_of_work" not in present:
            batch.add_column(
                sa.Column("session_kind_of_work", sa.String(32), nullable=True)
            )
        if "session_confirmation_line" not in present:
            batch.add_column(
                sa.Column("session_confirmation_line", sa.Text(), nullable=True)
            )
