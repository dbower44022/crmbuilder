...
``instance_memberships.member_type`` admits ``rule``; rebuilds
``ck_instance_membership_member_type`` from the current vocab (superset — no
row invalidated). No new entity type, no new relationship kind. Mirrors 0077.

PG chain head 0077 -> 0078. Companion SQLite-chain delta:
``migrations/versions/0078_pi_421_rule_membership.py``.
...
revision: str = "0078_pi_421_rule_membership"
down_revision: str | None = "0077_pi_420_message_template_membership"
