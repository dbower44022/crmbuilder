"""PI-195 (PG chain) — filtered_tabs table + CHECK rebuilds.

Companion to the SQLite-chain ``0061``. Creates the ``filtered_tabs`` table and
rebuilds the change_log / refs entity-type CHECKs and the
``instance_memberships`` member-type CHECK to admit the ``filtered_tab`` family
on Postgres deployments materialised from an earlier baseline. The PG baseline
is ``create_all`` from the live models, so a fresh PG DB already carries it — the
...
revision: str = "0019_pi_195_filtered_tab"
down_revision: str | None = "0018_pi_193_194_layout_role_team"
...
def upgrade() -> None:
    bind = op.get_bind()
    if FilteredTab.__tablename__ not in _tables():
        FilteredTab.__table__.create(bind)
