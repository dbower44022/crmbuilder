...
def master_conn(tmp_path):
    ...
    c.execute(
        "INSERT INTO Client (name, code, database_path, organization_overview, crm_platform, project_folder) "
        "VALUES ('Test Org', 'TO', '/tmp/test.db', 'We are a test org.', 'EspoCRM', '/tmp/test-project')"
    )
    c.commit()
    yield c
    c.close()
