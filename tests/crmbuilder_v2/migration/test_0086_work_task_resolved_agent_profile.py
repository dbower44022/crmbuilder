"""PI-302 — migration 0086 adds work_tasks.work_task_resolved_agent_profile.

create_all, drop the column, stamp 0085, upgrade 0086, assert the column is back;
then downgrade to 0085 and assert it is gone. The add is guarded so the migration is
...
_MIGRATION = "0086_pi_302_work_task_resolved_agent_profile"
...
def test_0086_adds_and_drops_resolved_agent_profile(tmp_path: Path) -> None:
    db = tmp_path / "v2.db"
