...
_DOWN = "0086_pi_444_instance_feature_selection"
_MIGRATION = "0087_pi_414_field_vocabulary_subtractive"
...
def _field(identifier: str, *, flag: int, fmt: str | None,
    ...
    return (
        ...
        f"('ENG-001', '{identifier}', 'f_{identifier}', 'd', 'text', false, "
        f"'candidate', false, false, false, {'true' if flag else 'false'}, {_lit(fmt)}, {_lit(display)}, "
        f"{_lit(supplied_by)}, '2026-01-01', '2026-01-01')"
    )


def test_0130_converts_then_subtracts() -> None:
    db = fresh_db()
