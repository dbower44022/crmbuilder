"""v0.2 slice B: integration tests for RisksPanel write surfaces."""
...
def _stub_record(identifier: str = "RSK-001") -> dict[str, Any]:
    return {
...
def _risks_only_handler(records: list[dict]):
    def handler(req: httpx.Request) -> httpx.Response:
        if req.method == "GET" and req.url.path == "/risks":
        ...
        if req.method == "GET" and req.url.path.startswith("/risks/"):
...
def test_toolbar_new_risk_button_present(qtbot):
    client = build_client(_risks_only_handler([]))
    panel = RisksPanel(client)
    qtbot.addWidget(panel)

    new_btn = panel.findChild(QPushButton, "new_risk_button")
    assert new_btn is not None
    assert new_btn.text() == "New Risk"


def test_toolbar_new_risk_button_opens_create_dialog(qtbot, monkeypatch):
    client = build_client(_risks_only_handler([]))
    panel = RisksPanel(client)
    qtbot.addWidget(panel)
    ...
    monkeypatch.setattr(
        "crmbuilder_v2.ui.panels.risks.RiskCreateDialog", _StubDialog
    )
...
def test_successful_create_calls_select_record_by_identifier(qtbot, monkeypatch):
    ...
    panel._on_new_risk_clicked()

    assert captured["selected"] == "RSK-100"


def test_detail_pane_has_edit_and_delete_buttons(qtbot):
    record = _stub_record()
    client = build_client(_risks_only_handler([record]))
    panel = RisksPanel(client)
    qtbot.addWidget(panel)
    ...
    edit_btn = detail.findChild(QPushButton, "edit_risk_button")
    delete_btn = detail.findChild(QPushButton, "delete_risk_button")
...
def test_edit_click_fetches_fresh_record_and_opens_edit_dialog(qtbot, monkeypatch):
    ...
    client = MagicMock()
    client.get_risk.return_value = fresh
    ...
    monkeypatch.setattr(
        "crmbuilder_v2.ui.panels.risks.RiskEditDialog", _StubEdit
    )

    panel._on_edit_clicked(record)

    client.get_risk.assert_called_once_with("RSK-001")
...
def test_successful_edit_triggers_panel_refresh(qtbot, monkeypatch):
    record = _stub_record()
    client = MagicMock()
    client.get_risk.return_value = record
...
def test_delete_click_opens_dialog_with_identifier_and_title(qtbot, monkeypatch):
    ...
    monkeypatch.setattr(
        "crmbuilder_v2.ui.panels.risks.RiskDeleteDialog", _StubDelete
    )

    panel._on_delete_clicked(record)

    assert captured["identifier"] == "RSK-001"
    assert captured["title"] == "Schema drift"
...
def test_risks_panel_right_click_invokes_context_menu_factory(
    qtbot, client_stub
):
